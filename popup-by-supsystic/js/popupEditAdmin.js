  ...
  jQuery("#ppsPopupPreviewFrame")
    .on("load", function () {
      ...
      var contentDoc = jQuery(this).contents(),
        ...
        paddingSize = 40,
        newWidth =
          jQuery(this).get(0).contentWindow.document.body.scrollWidth +
          paddingSize,
        newHeight =
          jQuery(this).get(0).contentWindow.document.body.scrollHeight +
          paddingSize,
        parentWidth = jQuery("#ppsPopupPreview").width(),
        widthMeasure = jQuery("#ppsPopupEditForm")
          .find('[name="params[tpl][width_measure]"]:checked')
          .val();
      ...
      if (1 == 0) {
        //if(in_array($this->popup['type'], array(PPS_FB_LIKE))
      ...
      if (typeof ppsPopup !== "undefined") {
        var addMovePopUps = [
          // Additional preview PopUps movements
        ...
        for (var i = 0; i < addMovePopUps.length; i++) {
          if (
            ppsPopup.id == addMovePopUps[i].id ||
