...
jQuery(document).ready(function () {
  jQuery('#ppsPopupEditTabs').wpTabs({
    ...
    change: function (selector) {
      ...
      if (selector == '#ppsPopupStatistics') {
  ...
  jQuery('#ppsPopupEditForm').submit(function () {
    ...
    if (!ppsSaveWithoutPreviewUpdate) ppsShowPreviewUpdating();
    ...
    jQuery(this).sendFormPps({
      ...
      onSuccess: function (res) {
        ppsPopupIsSaving = false;
        if (!res.error) {
          if (!ppsSaveWithoutPreviewUpdate) ppsRefreshPreview();
        }
        ppsSaveWithoutPreviewUpdate = false;
        if (cssSet && cssEditor) {
          jQuery('#ppsPopupCssEditor').val(cssEditor.getValue());
  ...
  jQuery('#ppsPopupEditForm')
    .find('input[name="params[main][show_on]"],input[name="params[main][close_on]"],input[name="params[main][show_to]"],input[name="params[main][show_pages]"]')
    .on('change', function () {
  ...
  var animSelector = {
    ...
    _setAnim: function (key) {
      ...
      jQuery('#ppsPopupEditForm')
        .find(this._forClose ? '[name="params[tpl][anim_close_key]"]' : '[name="params[tpl][anim_key]"]')
        .val(key);
      jQuery('#ppsPopupAnimCurrStyle').html(animElement.data('label'));
    },
    _switchClose: function () {
      this._forClose = !this._forClose;
      var animKey = jQuery('#ppsPopupEditForm')
        .find(this._forClose ? '[name="params[tpl][anim_close_key]"]' : '[name="params[tpl][anim_key]"]')
        .val();
      if (!animKey || animKey == '') animKey = 'none';
  ...
  jQuery('.ppsPopupRemoveBtn').click(function () {
    if (confirm(toeLangPps('Are you sure want to remove this Pop-Up?'))) {
      jQuery.sendFormPps({
        btn: this,
        data: { mod: 'popup', action: 'remove', _wpnonce: PPS_NONCE['pps_nonce'], id: ppsPopup.id },
        onSuccess: function (res) {
          if (!res.error) {
  ...
  setTimeout(function () {
    ...
    for (var i = 0; i < autoUpdateBoxes.length; i++) {
      jQuery(autoUpdateBoxes[i])
        .find('input[type=checkbox],input[type=radio],input[type=hidden],select')
        .not(ignoreInputs)
        .change(function () {
          ppsMakeAutoUpdate(1);
        });
  ...
  jQuery('.ppsPopupSwitchActive').click(function () {
    ...
    jQuery.sendFormPps({
      btn: this,
      data: { mod: 'popup', action: 'switchActive', _wpnonce: PPS_NONCE['pps_nonce'], id: ppsPopup.id, active: newActive },
      onSuccess: function (res) {
        if (!res.error) {
  ...
  jQuery('#supsystic-breadcrumbs').bind('startSticky', function () {
    ...
    jQuery('#ppsPopupMainControllsShell')
      .css('padding-right', currentPadding + 200)
      .attr('data-padding-changed', 'padding is changed in admin.popup.edit.js');
  });
  ...
  jQuery('.ppsSwitchShowHideOptLink').each(function () {
    ...
    if (toeInArray(inputName, parsedShowHideInputNames) === -1) {
      ...
      jQuery(this)
        .parents('.ppsPopupMainOptLbl:first')
        .find('.ppsSwitchShowHideOptLink[data-input-value="' + inputVal + '"]')
        .addClass('active');
  ...
  jQuery('#ppsPopupEditForm [name="params[main][enb_show_time]"]')
    .change(function () {
      if (jQuery(this).prop('checked')) {
  ...
  jQuery('#ppsPopupEditForm [name="params[main][enb_show_date]"]')
    .change(function () {
      if (jQuery(this).prop('checked')) {
  ...
  jQuery('#ppsPopupEditForm [name="params[main][enb_show_days]"]')
    .change(function () {
      if (jQuery(this).prop('checked')) {
  ...
  // Edit PopUp Label
  jQuery('#ppsPopupEditableLabelTxt')
    .blur(function () {
  ...
  jQuery('.ppsPopupAttachFilesShell').each(function () {
    ...
    if (ppsPopup.params && ppsPopup.params.tpl && ppsPopup.params.tpl[filesKey]) {
  ...
  jQuery('#ppsPopupEditForm input[name="params[main][show_on_page_load_enb_delay]"]')
    .change(function () {
      jQuery(this).prop('checked') ? jQuery('.ppsPageGlobalDelayShell').slideDown(g_ppsAnimationSpeed) : jQuery('.ppsPageGlobalDelayShell').slideUp(g_ppsAnimationSpeed);
    })
    .change();
  // Bg overlay types change
  jQuery('#ppsPopupEditForm [name="params[tpl][bg_overlay_type]"]')
    .change(function () {
...
function ppsFinishEditPopupLabel(label) {
  ...
  jQuery.sendFormPps({
    ...
    data: { mod: 'popup', action: 'updateLabel', _wpnonce: PPS_NONCE['pps_nonce'], label: label, id: ppsPopup.id },
    onSuccess: function (res) {
      if (!res.error) {
        var $labelHtml = jQuery('#ppsPopupEditableLabel'),
          $labelTxt = jQuery('#ppsPopupEditableLabelTxt');
...
function ppsAdjustPopupsEditTabs(requring) {
  jQuery('#ppsPopupEditTabs .supsystic-always-top').outerWidth(jQuery('#ppsPopupEditTabs').width()).attr('data-code-tip', 'Width was set in admin.popup.edit.js - ppsAdjustPopupsEditTabs()');
...
function ppsSavePopupChanges(withoutPreviewUpdate) {
  // Triger save
  if (withoutPreviewUpdate) ppsSaveWithoutPreviewUpdate = true;
...
function ppsMakeAutoUpdate(delay) {
  ...
  if (ppsPopupSaveTimeout) clearTimeout(ppsPopupSaveTimeout);
...
function _ppsPopupHideIpMoveFromText(notUserOpen) {
  ...
  if (!ipsArr || !ipsArr.length) ipsArr = false;
  if (ipsArr) {
    jQuery.map(ipsArr, jQuery.trim);
  }
  if (!notUserOpen)
    jQuery('#ppsPopupEditForm')
      .find('[name="params[main][hide_for_ips]"]')
      .val(ipsArr ? ipsArr.join(',') : '')
      .trigger('change');
  jQuery('#ppsHiddenIpStaticList').html(ipsArr ? ipsArr.length + ' ' + toeLangPps('IPs are blocked') : toeLangPps('No IPs are currently in block list'));
}
