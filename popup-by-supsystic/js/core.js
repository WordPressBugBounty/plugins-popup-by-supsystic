...
if (isNumber(PPS_DATA.animationSpeed)) PPS_DATA.animationSpeed = parseInt(PPS_DATA.animationSpeed);
else if (jQuery.inArray(PPS_DATA.animationSpeed, ['fast', 'slow']) == -1) PPS_DATA.animationSpeed = 'fast';
...
jQuery.fn.sendFormPps = function (params) {
  ...
  else if (typeof ajaxurl == 'undefined' || typeof ajaxurl !== 'string') url = PPS_DATA.ajaxurl;
  else url = ajaxurl;
...
function toeLangPps(key) {
  if (PPS_DATA.siteLang && PPS_DATA.siteLang[key]) return PPS_DATA.siteLang[key];
  return key;
}
...
function toeOptTimeoutHideDescriptionPps() {
  ...
  setTimeout(function () {
    if (!jQuery('#ppsOptDescription').attr('toeFixTip')) toeOptHideDescriptionPps();
  }, 500);
}
/**
...
function toeOptShowDescriptionPps(description, x, y, moveToLeft) {
  if (typeof description != 'undefined' && description != '') {
    ...
    if (moveToLeft) jQuery('#ppsOptDescription').css('right', jQuery(window).width() - (x - 10));
