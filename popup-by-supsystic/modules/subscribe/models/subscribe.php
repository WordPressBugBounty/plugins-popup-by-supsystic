...
class subscribeModelPps extends modelPps
{
  ...
  public function subscribe($d = [], $validateIp = false)
  {
    $id = isset($d['id']) ? $d['id'] : 0;
    if ($id) {
      $popup = framePps::_()->getModule('popup')->getModel()->getById($id);
      if ($popup && isset($popup['params']) && isset($popup['params']['tpl']['enb_subscribe']) && isset($popup['params']['tpl']['sub_dest'])) {
        ...
        if ($this->_subscribedInternal || $externalModel) {
          ...
          if ($this->validateFields($d, $popup)) {
            $res = $this->_subscribedInternal ? $this->$subMethod($d, $popup, $validateIp) : $externalModel->subscribe($d, $popup, $validateIp);
  ...
  {
    if (isset($popup['params']['tpl']['sub_create_wp_user']) && $popup['params']['tpl']['sub_create_wp_user'] && $dest != 'wordpress') {
  ...
  public function checkSendNewNotification($d, $popup, $forReg = false)
  {
    ...
    if (!isset($popup['params']['tpl'][$pref . '_new_email'])) {
      ...
      $sendTo = isset($popup['params']['tpl'][$pref . '_new_email']) && !empty($popup['params']['tpl'][$pref . '_new_email']) ? trim($popup['params']['tpl'][$pref . '_new_email']) : false;
    }
    if (!empty($sendTo)) {
      ...
      $emailSubject = empty($blogName) ? $defSubject : sprintf(__('New Subscriber on %s', PPS_LANG_CODE), $blogName);
      if (isset($popup['params']['tpl'][$pref . '_new_subject']) && !empty($popup['params']['tpl'][$pref . '_new_subject'])) {
      ...
      $emailContent = isset($popup['params']['tpl'][$pref . '_new_message']) && !empty($popup['params']['tpl'][$pref . '_new_message']) ? $popup['params']['tpl'][$pref . '_new_message'] : $defContent;
      $subscriberDataArr = [];
      if (isset($popup['params']['tpl'][$pref . '_fields']) && !empty($popup['params']['tpl'][$pref . '_fields'])) {
        foreach ($popup['params']['tpl'][$pref . '_fields'] as $k => $f) {
          if (isset($d[$k])) {
            $value = $d[$k];
            if ($k == 'mailchimp_goup') {
              if (isset($popup['params']['tpl']['sub_mailchimp_groups_full']) && !empty($popup['params']['tpl']['sub_mailchimp_groups_full'])) {
                $mcGoups = explode(';', $popup['params']['tpl']['sub_mailchimp_groups_full']);
                foreach ($mcGoups as $g) {
                  $gIdLabel = explode(':', $g);
                  if ($gIdLabel[0] == $value) {
                    // Find MailChimp Group by it's selected ID
                    $value = $gIdLabel[1];
                    break;
                  }
                }
      ...
      framePps::_()->getModule('mail')->send($sendTo, $emailSubject, $emailContent, $blogName, $adminEmail, $blogName, $adminEmail, [], $addMailParams);
    }
  }
  ...
  public function validateFields($d, $popup, $forReg = false)
  {
    ...
    if (isset($popup['params']['tpl'][$pref . '_fields']) && !empty($popup['params']['tpl'][$pref . '_fields'])) {
      ...
      if (isset($popup['params']['tpl']['blacklist']) && !empty($popup['params']['tpl']['blacklist']) && framePps::_()->getModule('add_options') && method_exists(framePps::_()->getModule('add_options'), 'checkEmailBlackList')) {
        $email = isset($popup['params']['tpl'][$pref . '_fields']['email']) && isset($d['email']) ? $d['email'] : '';
        if (!empty($email)) {
          if (
            framePps::_()
              ->getModule('add_options')
              ->checkEmailBlackList($email, $popup['params']['tpl']['blacklist'])
          ) {
            $errors['email'] = isset($popup['params']['tpl']['blacklist_error']) && !empty($popup['params']['tpl']['blacklist_error']) ? $popup['params']['tpl']['blacklist_error'] : __('Your email is in blacklist', PPS_LANG_CODE);
          }
        }
      }
      if (!empty($errors)) {
   ...
   */
  public function checkOftenAccess($d = [])
  {
    return $this->_checkOftenAccess($d);
  }
  private function _checkOftenAccess($d = [])
  {
  ...
  private function _getInvalidEmailMsg($popup = [], $forReg = false, $existsProblem = false)
  {
    $pref = $forReg ? 'reg' : 'sub';
    $msg = !empty($popup) && isset($popup['params']['tpl'][$pref . '_txt_invalid_email']) ? $popup['params']['tpl'][$pref . '_txt_invalid_email'] : __('Empty or invalid email', PPS_LANG_CODE);
    if ($existsProblem && !empty($popup) && isset($popup['params']['tpl'][$pref . '_txt_exists_email']) && !empty($popup['params']['tpl'][$pref . '_txt_exists_email'])) {
    ...
    if ($userId && !is_wp_error($userId)) {
      ...
      if (isset($popup['params']['tpl'][$pref . '_fields']) && !empty($popup['params']['tpl'][$pref . '_fields'])) {
        foreach ($popup['params']['tpl'][$pref . '_fields'] as $k => $f) {
          ...
          if (isset($d[$k])) {
            wp_update_user(['ID' => $userId, $k => $d[$k]]);
          }
        }
      }
  ...
  private function _sendNewUserNotification($popup, $userId, $password, $d, $forReg = false)
  {
    ...
    if ($emailSubject && $emailContent) {
      ...
      $blogName = wp_specialchars_decode(get_bloginfo('name'));
      $blogName = str_replace('&#039;', "'", $blogName);
      $adminEmail = isset($popup['params']['tpl'][$pref . '_txt_subscriber_mail_from']) ? $popup['params']['tpl'][$pref . '_txt_subscriber_mail_from'] : get_bloginfo('admin_email');
      $replaceVariables = [
        'sitename' => $blogName,
        'siteurl' => get_bloginfo('wpurl'),
      ...
      framePps::_()->getModule('mail')->send($user->user_email, $emailSubject, $emailContent, $blogName, $adminEmail, $blogName, $adminEmail, [], $addMailParams);
    } else {
      // Just use standard wp method
      if (
        isset($popup['params']['tpl'][$pref . '_txt_subscriber_mail_subject']) &&
  ...
  public function sendWpUserConfirm($username, $email, $confirmHash, $popup, $forReg = false, $d = [])
  {
    $pref = $forReg ? 'reg' : 'sub';
    $blogName = wp_specialchars_decode(get_bloginfo('name'));
    $blogName = str_replace('&#039;', "'", $blogName);
    ...
    $adminEmail = isset($popup['params']['tpl'][$pref . '_txt_confirm_mail_from']) ? $popup['params']['tpl'][$pref . '_txt_confirm_mail_from'] : get_bloginfo('admin_email');
    $confirmSubject = isset($popup['params']['tpl'][$pref . '_txt_confirm_mail_subject']) && !empty($popup['params']['tpl'][$pref . '_txt_confirm_mail_subject']) ? $popup['params']['tpl'][$pref . '_txt_confirm_mail_subject'] : __('Confirm subscription on [sitename]', PPS_LANG_CODE);
    $confirmContent =
      isset($popup['params']['tpl'][$pref . '_txt_confirm_mail_message']) && !empty($popup['params']['tpl'][$pref . '_txt_confirm_mail_message'])
    ...
    framePps::_()->getModule('mail')->send($email, $confirmSubject, $confirmContent, $blogName, $adminEmail, $blogName, $adminEmail, [], $addMailParams);
  }
  ...
  public function confirm($d = [], $forReg = false)
  {
    ...
    if (!empty($d['email']) && !empty($d['hash'])) {
      ...
      if (!empty($subscriber)) {
        ...
        $subscriber['all_data'] = isset($subscriber['all_data']) ? utilsPps::unserialize($subscriber['all_data']) : [];
        $res = $this->createWpSubscriber($popup, $subscriber['email'], $subscriber['username'], $subscriber['all_data'], $forReg);
        if ($res) {
          $subscribeModel->update(['activated' => 1], ['id' => $subscriber['id']]);
        }
        return $res;
      }
    }
  ...
  public function getMailchimpLists($d = [])
  {
    ...
    if (!empty($key)) {
      ...
      if ($this->_useMailchimp3Version) {
        $apiRes = $client->get('lists', ['count' => 100]);
        if ($apiRes && isset($apiRes['lists']) && !empty($apiRes['lists'])) {
          $lists = $apiRes['lists'];
        }
      } else {
        $apiRes = $client->call('lists/list', ['limit' => 100, 'sort_field' => 'web']);
  ...
  public function getMailchimpGroups($d = [])
  {
    if (!$this->isMailchimpSupported()) {
      return false;
    }
    $key = isset($d['key']) ? trim($d['key']) : '';
    $listIds = isset($d['listIds']) && is_array($d['listIds']) && !empty($d['listIds']) ? array_map('trim', $d['listIds']) : false;
    if (!empty($key)) {
      if (!empty($listIds)) {
        $client = $this->_getMailchimpInst($key);
        $groupsRes = [];
        foreach ($listIds as $lid) {
          $groupsInterests = [];
          if ($this->_useMailchimp3Version) {
            $apiRes = $client->get('/lists/' . $lid . '/interest-categories', ['count' => 100]);
            if ($apiRes && is_array($apiRes) && isset($apiRes['categories']) && !empty($apiRes['categories'])) {
              $groupsInterests = $apiRes['categories'];
            }
          } else {
            $apiRes = $client->call('lists/interest-groupings', ['id' => $lid]);
          ...
          if ($apiRes && is_array($apiRes) && !isset($apiRes['errors']) && !isset($apiRes['error'])) {
            foreach ($groupsInterests as $pGroup) {
              $groups = [];
              if ($this->_useMailchimp3Version) {
                $groupsApiRes = $client->get('lists/' . $lid . '/interest-categories/' . $pGroup['id'] . '/interests', ['count' => 100]);
              ...
              if (!empty($groups)) {
                foreach ($groups as $group) {
                  $groupsRes[$lid . '|' . $pGroup['id'] . '|' . base64_encode($this->_useMailchimp3Version ? $group['id'] : $group['name'])] = ($this->_useMailchimp3Version ? $pGroup['title'] : $pGroup['name']) . ' - ' . $group['name'];
                }
              }
            }
          } else {
  ...
  public function subscribe_mailchimp($d, $popup, $validateIp = false)
  {
    ...
    if (!empty($email)) {
      if (is_email($email)) {
        ...
        $lists = isset($popup['params']['tpl']['sub_mailchimp_lists']) ? $popup['params']['tpl']['sub_mailchimp_lists'] : [];
        $apiKey = isset($popup['params']['tpl']['sub_mailchimp_api_key']) ? trim($popup['params']['tpl']['sub_mailchimp_api_key']) : [];
        // Check for mailchimp list select field
        if (
          framePps::_()->getModule('supsystic_promo')->isPro() &&
          ...
          $lists = [$d['mailchimp_list']];
        }
        if (!empty($lists)) {
          if (!empty($apiKey)) {
            if (!$validateIp || ($validateIp && $this->_checkOftenAccess(['only_check' => true]))) {
              $name = '';
              ...
              $dataToSend = ['email' => $member];
              if (!empty($name)) {
                $firstLastNames = array_map('trim', explode(' ', $name));
                $dataToSend['merge_vars'] = [
              ...
              foreach ($lists as $listId) {
                ...
                if (isset($dataToSend['merge_vars']['groupings'])) {
                  // It can be set from prev. subscribe list
                  ...
                  if (isset($d['mailchimp_goup'])) {
                    ...
                    if (count($listParentGroupGroupIds) == 3) {
                      if (!isset($grouping[$listParentGroupGroupIds[1]])) {
                        $grouping[$listParentGroupGroupIds[1]] = ['id' => $listParentGroupGroupIds[1], 'groups' => []];
                      }
                ...
                if ($this->_useMailchimp3Version) {
                  ...
                  if (isset($dataToSend['merge_vars']) && !empty($dataToSend['merge_vars'])) {
                    ...
                    if (isset($dataToSend['merge_vars']['groupings']) && !empty($dataToSend['merge_vars']['groupings'])) {
              ...
              if ($validateIp) {
                $this->_checkOftenAccess(['only_add' => true]);
              }
              return true;
            }
          } else {
  ...
  public function getMailPoetLists($version = false)
  {
    ...
    if ($version) {
      switch ($version) {
        case 2:
          $lists = WYSIJA::get('list', 'model')->get(['name', 'list_id'], ['is_enabled' => 1]);
          break;
        case 3:
        ...
        if ($mailPoetVer) {
          ...
          switch ($mailPoetVer) {
            case 2:
              $dataSubscriber = [
                'user' => $userData,
                'user_list' => ['list_ids' => [$popup['params']['tpl']['sub_mailpoet_list']]],
              ];
              if (!empty($userFields)) {
            ...
            case 3:
              ...
              try {
                $this->_getMailPoetV3Engine();
                if ($this->_mailPoetV3Engine) {
                  $subscriber = $this->_mailPoetV3Engine->subscribe($userData, [$popup['params']['tpl']['sub_mailpoet_list']], $options);
                  if ($subscriber) {
                    if ($validateIp) {
                      $this->_checkOftenAccess(['only_add' => true]);
                    }
    ...
    if (!empty($email) && is_email($email)) {
      if (!$validateIp || ($validateIp && $this->_checkOftenAccess(['only_check' => true]))) {
        if (class_exists('frameNbs')) {
          ...
          if (isset($popup['params']['tpl']['sub_sup_send_confirm']) && $popup['params']['tpl']['sub_sup_send_confirm']) {
      ...
      if (!$validateIp || ($validateIp && $this->_checkOftenAccess(['only_check' => true]))) {
        if (class_exists('Jetpack')) {
          if (class_exists('Jetpack_Subscriptions')) {
            ...
            $jetRes = $jetSubMod->subscribe($email, 0, false, [
