...
class fieldAdapterPps
{
  ...
  public static function taxDataHtml($field)
  {
    ...
    return '<div>' .
      __('Apply To', PPS_LANG_CODE) .
      '
            <div id="tax_address">
                <b>' .
      __('Address', PPS_LANG_CODE) .
      '</b><br />
                ' .
      __('Destination', PPS_LANG_CODE) .
      ':' .
      htmlPps::checkboxlist('params[dest]', ['optionsPps' => $listOfDest]) .
      '<br />
                ' .
      __('Country', PPS_LANG_CODE) .
      ':' .
      htmlPps::countryList('params[country]', ['notSelected' => true, 'value' => $field->value['country']]) .
      '<br />
            </div>
            <div id="tax_category">
                <b>' .
      __('Categories', PPS_LANG_CODE) .
      '</b><br />
                ' .
      (empty($cOptions) ? __('You have no categories', PPS_LANG_CODE) : htmlPps::checkboxlist('params[categories][]', ['optionsPps' => $cOptions])) .
      '<br />
                    <b>' .
      __('Brands', PPS_LANG_CODE) .
      '</b><br />
                ' .
      (empty($bOptions) ? __('You have no brands', PPS_LANG_CODE) : htmlPps::checkboxlist('params[brands][]', ['optionsPps' => $bOptions])) .
      '<br />
            </div>
            <div>' .
      __('Tax Rate', PPS_LANG_CODE) .
      ': ' .
      htmlPps::text('params[rate]', ['value' => $field->value['rate']]) .
      '</div>
            <div>' .
      __('Absolute', PPS_LANG_CODE) .
      ': ' .
      htmlPps::checkbox('params[absolute]', ['checked' => $field->value['absolute']]) .
      '</div>
        </div>';
  }
  ...
  public static function getFieldAttributes($params)
  {
    ...
    if (!empty($params->attr)) {
      foreach ($params->attr as $key => $value) {
        $output .= __($key) . ':<br />';
        $output .= htmlPps::text('params[attr][' . $key . ']', ['value' => $value]) . '<br />';
      }
    } else {
      $output .= __('class', PPS_LANG_CODE) . ':<br />';
      $output .= htmlPps::text('params[attr][class]', ['value' => '']) . '<br />';
      $output .= __('id', PPS_LANG_CODE) . ':<br />';
      $output .= htmlPps::text('params[attr][id]', ['value' => '']) . '<br />';
    }
    return $output;
  }
  /**
  ...
  public static function getCachedStates($clearCache = false)
  {
    if (empty(self::$states) || $clearCache) {
      self::$states = framePps::_()
        ->getTable('states')
        ->leftJoin(framePps::_()->getTable('countries'), 'country_id')->getAll('toe_states.id,
					toe_states.name, 
					toe_states.code, 
