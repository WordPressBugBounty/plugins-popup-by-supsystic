...
class popupModelPps extends modelPps
{
  ...
  private function _getLinksReplacement()
  {
    if (empty($this->_linksReplacement)) {
      $this->_linksReplacement = [
        'modUrl' => ['url' => $this->getModule()->getModPath(), 'key' => 'PPS_MOD_URL'],
        'siteUrl' => ['url' => PPS_SITE_URL, 'key' => 'PPS_SITE_URL'],
        'assetsUrl' => ['url' => $this->getModule()->getAssetsUrl(), 'key' => 'PPS_ASSETS_URL'],
        'oldAssets' => ['url' => $this->getModule()->getOldAssetsUrl(), 'key' => 'PPS_OLD_ASSETS_URL'],
      ];
    }
    ...
    if (is_array($data)) {
      ...
      if (!$replaceFrom) {
        ...
        foreach ($this->_linksReplacement as $k => $rData) {
          if ($k == 'oldAssets') {
            // Replace old assets urls - to new one
  ...
  protected function _afterGetFromTbl($row)
  {
    if (isset($row['params'])) {
      ...
      if (!empty($row['params']) && isset($row['params']['tpl']) && !empty($row['params']['tpl'])) {
        $keysCheckEmail = ['sub_txt_subscriber_mail_from', 'sub_new_email', 'sub_txt_confirm_mail_from', 'reg_txt_subscriber_mail_from', 'reg_txt_confirm_mail_from', 'reg_new_email'];
    ...
    if (!isset($row['params']['tpl']['sub_fields'])) {
      $row['params']['tpl']['sub_fields'] = [
        'email' => ['label' => __('E-Mail', PPS_LANG_CODE), 'html' => 'text', 'enb' => true, 'mandatory' => true, 'name' => 'email'],
        'name' => ['label' => __('Name', PPS_LANG_CODE), 'html' => 'text', 'enb' => isset($row['params']['tpl']['enb_sub_name']) && $row['params']['tpl']['enb_sub_name'], 'name' => 'name'],
      ];
    } else {
      // Saving Enabling name field for old field database sctructure - to not lose this field for old popups
      if (isset($row['params']['tpl']['sub_fields']) && isset($row['params']['tpl']['sub_fields']['name']) && isset($row['params']['tpl']['sub_fields']['name']['enb']) && $row['params']['tpl']['sub_fields']['name']['enb']) {
        $row['params']['tpl']['enb_sub_name'] = 1;
      }
    }
    // For PRO only, but let it be here for now
    if (!isset($row['params']['tpl']['reg_fields'])) {
      $row['params']['tpl']['reg_fields'] = [
        'name' => ['label' => __('Name', PPS_LANG_CODE), 'html' => 'text', 'enb' => 1, 'name' => 'name'],
        'email' => ['label' => __('E-Mail', PPS_LANG_CODE), 'html' => 'text', 'enb' => true, 'mandatory' => true, 'name' => 'email'],
      ];
    }
    if ($row['type'] == PPS_VIDEO && !isset($row['params']['tpl']['video_type'])) {
      $videoTypes = [PPS_VIDEO_YOUTUBE, PPS_VIDEO_VIMEO];
      $row['params']['tpl']['video_type'] = PPS_VIDEO_OTHER;
      foreach ($videoTypes as $t) {
        if (isset($row['params']['tpl']['video_url']) && strpos($row['params']['tpl']['video_url'], $t) !== false) {
  ...
  public function createFromTpl($d = [])
  {
    ...
    if (!empty($d['label'])) {
      if (!empty($d['original_id'])) {
        ...
        if (in_array($original['type'], [PPS_VIDEO])) {
  ...
  protected function _socShareGetSearchString($id)
  {
    ...
    $serialized = serialize(['popup_id' => $id]);
  ...
  private function _bindShowToPages($d)
  {
    ...
    if ($id) {
      ...
      framePps::_()
        ->getTable('popup_show_categories')
        ->delete(['popup_id' => $id]);
      $insertArr = [];
  ...
  public function getShowToList()
  {
    if (empty($this->_showToList)) {
      $this->_showToList = [
        'everyone' => ['id' => 1],
        'first_time_visit' => ['id' => 2],
        'for_countries' => ['id' => 3],
        'until_make_action' => ['id' => 4],
        'count_times' => ['id' => 5],
        'until_email_confirm' => ['id' => 6],
      ];
    }
  ...
  public function getShowPagesList()
  {
    if (empty($this->_showPagesList)) {
      $this->_showPagesList = [
        'all' => ['id' => 1],
        'show_on_pages' => ['id' => 2],
        'not_show_on_pages' => ['id' => 3],
        'show_on_categories' => ['id' => 4],
        'not_show_on_categories' => ['id' => 5],
      ];
    }
  ...
  public function getShowOnList()
  {
    if (empty($this->_showOnList)) {
      $this->_showOnList = dispatcherPps::applyFilters('popupShowOnList', [
        'page_load' => ['id' => 1],
        'click_on_page' => ['id' => 2],
        'click_on_element' => ['id' => 3],
        'scroll_window' => ['id' => 4],
        'on_exit' => ['id' => 5],
        'page_bottom' => ['id' => 6],
        'after_inactive' => ['id' => 7],
  ...
  public function getById($id)
  {
    ...
    if ($data) {
      $data['show_pages_list'] = framePps::_()
        ->getTable('popup_show_pages')
        ->get('*', ['popup_id' => $id]);
      $data['show_categories_list'] = framePps::_()
        ->getTable('popup_show_categories')
        ->get('*', ['popup_id' => $id]);
    }
    return $data;
  ...
  public function getTypes()
  {
    if (empty($this->_types)) {
      $this->_types = dispatcherPps::applyFilters('popupTypesList', [
        1 => ['code' => PPS_COMMON, 'label' => __('Common', PPS_LANG_CODE)],
        2 => ['code' => PPS_FB_LIKE, 'label' => __('Facebook Like', PPS_LANG_CODE)],
        3 => ['code' => PPS_VIDEO, 'label' => __('Video', PPS_LANG_CODE)],
        4 => ['code' => PPS_IFRAME, 'label' => __('iFrame', PPS_LANG_CODE)],
        5 => ['code' => PPS_SIMPLE_HTML, 'label' => __('Simple HTML', PPS_LANG_CODE)],
        6 => ['code' => PPS_PDF, 'label' => __('PDF', PPS_LANG_CODE)],
        7 => ['code' => PPS_AGE_VERIFY, 'label' => __('Age Verification', PPS_LANG_CODE)],
        8 => ['code' => PPS_FULL_SCREEN, 'label' => __('Full Screen', PPS_LANG_CODE)],
        9 => ['code' => PPS_LOGIN_REGISTER, 'label' => __('Login / Registration', PPS_LANG_CODE)],
        10 => ['code' => PPS_BAR, 'label' => __('Notification Bar', PPS_LANG_CODE)],
        11 => ['code' => PPS_COMMON, 'label' => __('Christmas', PPS_LANG_CODE)],
      ]);
    }
    return $this->_types;
  }
    ...
    if ($d['id'] && $d['new_tpl_id']) {
      ...
      if (!empty($diffFromOriginal)) {
        if (isset($newTpl['params'])) {
          $keysForMove = ['params.tpl.label', 'params.tpl.anim_key', 'params.tpl.enb_foot_note', 'params.tpl.foot_note', 'params.tpl.enb_sm', 'params.tpl.enb_subscribe'];
  ...
  private function _computeDifferences($popup, $original, $key = '', $keysImplode = [])
  {
    $difs = [];
    if (is_array($popup)) {
      $excludeKey = ['id', 'label', 'active', 'original_id', 'img_preview', 'type_id', 'date_created', 'view_id', 'img_preview_url', 'show_on', 'show_to', 'show_pages'];
      if (!empty($key)) {
        $keysImplode[] = $key;
      }
    ...
    if (!$recalculated) {
      ...
      if (!empty($allPopups)) {
        ...
        foreach ($allPopups as $p) {
          ...
          $stats = $statsModel->getPreparedStats(['id' => $p['id']]);
          if (!empty($stats)) {
  ...
  {
    return [
      1 => [__('Left side background', PPS_LANG_CODE), __('Right side background', PPS_LANG_CODE), __('Subscribe button background', PPS_LANG_CODE)],
      2 => [__('Background', PPS_LANG_CODE), __('Sign up button background', PPS_LANG_CODE)],
      3 => [__('Background', PPS_LANG_CODE), __('Button background', PPS_LANG_CODE), __('Right side background', PPS_LANG_CODE)],
      5 => [__('Left side background', PPS_LANG_CODE), __('Right side background', PPS_LANG_CODE), __('Button background', PPS_LANG_CODE)],
      6 => [__('Background', PPS_LANG_CODE), __('Button background', PPS_LANG_CODE)],
      7 => [__('Main Background', PPS_LANG_CODE), __('Down background', PPS_LANG_CODE), __('Button background', PPS_LANG_CODE)],
      9 => [__('Background', PPS_LANG_CODE), __('Button background', PPS_LANG_CODE)],
      10 => [__('Background', PPS_LANG_CODE), __('Button background', PPS_LANG_CODE)],
      11 => [__('Left Side Background', PPS_LANG_CODE), __('Right Side background', PPS_LANG_CODE), __('Button background', PPS_LANG_CODE)],
      12 => [__('Background', PPS_LANG_CODE), __('Button background', PPS_LANG_CODE)],
      13 => [__('Background', PPS_LANG_CODE), __('Sign-up button background', PPS_LANG_CODE), __('Close button background', PPS_LANG_CODE)],
      14 => [__('Main background', PPS_LANG_CODE), __('Middle side background', PPS_LANG_CODE), __('Down side background', PPS_LANG_CODE)],
      15 => [__('Main background', PPS_LANG_CODE), __('Fields background', PPS_LANG_CODE), __('Right side background', PPS_LANG_CODE), __('Button background', PPS_LANG_CODE)],
      16 => [__('Main background', PPS_LANG_CODE), __('Left picture background', PPS_LANG_CODE), __('Background image', PPS_LANG_CODE), __('Button background', PPS_LANG_CODE), __('Additional background', PPS_LANG_CODE)],
      17 => [__('Main background', PPS_LANG_CODE), __('Button background', PPS_LANG_CODE), __('Frame background', PPS_LANG_CODE)],
      18 => [__('Main background', PPS_LANG_CODE), __('Button background', PPS_LANG_CODE), __('Middle side background', PPS_LANG_CODE)],
      19 => [__('Main background', PPS_LANG_CODE), __('Additional background', PPS_LANG_CODE), __('Button background', PPS_LANG_CODE)],
      20 => [__('Main background', PPS_LANG_CODE), __('Exit button background', PPS_LANG_CODE), __('Left image background', PPS_LANG_CODE), __('Submit button background', PPS_LANG_CODE), __('Additional background', PPS_LANG_CODE), __('Right image background', PPS_LANG_CODE)],
      21 => [__('Main background', PPS_LANG_CODE), __('Button background', PPS_LANG_CODE), __('Left side background', PPS_LANG_CODE)],
      22 => [__('Left side background', PPS_LANG_CODE), __('Right side background', PPS_LANG_CODE), __('Button background', PPS_LANG_CODE), __('Image background', PPS_LANG_CODE)],
      23 => [__('Main background', PPS_LANG_CODE), __('Field background', PPS_LANG_CODE), __('Image background', PPS_LANG_CODE), __('Go button background', PPS_LANG_CODE)],
      24 => [__('Main background', PPS_LANG_CODE), __('Close button background', PPS_LANG_CODE), __('Middle side background', PPS_LANG_CODE), __('Submit button background', PPS_LANG_CODE)],
      25 => [__('Main background', PPS_LANG_CODE), __('Submit button background', PPS_LANG_CODE), __('Right Image background', PPS_LANG_CODE), __('Right background', PPS_LANG_CODE)],
      26 => [__('Main background', PPS_LANG_CODE), __('Button background', PPS_LANG_CODE), __('Image background', PPS_LANG_CODE)],
      27 => [__('Left side background', PPS_LANG_CODE), __('Right side background', PPS_LANG_CODE), __('Button background', PPS_LANG_CODE)],
      28 => [__('Main background', PPS_LANG_CODE), __('Field background', PPS_LANG_CODE), __('Left image background', PPS_LANG_CODE), __('Get it now button background', PPS_LANG_CODE)],
      29 => [__('Main background', PPS_LANG_CODE), __('Button background', PPS_LANG_CODE)],
      30 => [__('Main background', PPS_LANG_CODE), __('Upper background', PPS_LANG_CODE), __('Right image background', PPS_LANG_CODE), __('Bottom background', PPS_LANG_CODE), __('Button background', PPS_LANG_CODE)],
      31 => [__('Main background', PPS_LANG_CODE), __('Button background', PPS_LANG_CODE)],
      33 => [__('Main background', PPS_LANG_CODE), __('Frame background', PPS_LANG_CODE)],
      34 => [__('Main background', PPS_LANG_CODE), __('Frame background', PPS_LANG_CODE)],
      43 => [__('Main background', PPS_LANG_CODE), __('Field background', PPS_LANG_CODE), __('Image', PPS_LANG_CODE), __('Button Background', PPS_LANG_CODE), __('Button text color', PPS_LANG_CODE), __('Email text color', PPS_LANG_CODE), __('Header text color 2', PPS_LANG_CODE)],
      44 => [__('Main background', PPS_LANG_CODE), __('Image 1', PPS_LANG_CODE), __('Image 2', PPS_LANG_CODE), __('Button background', PPS_LANG_CODE), __('Email background', PPS_LANG_CODE)],
      45 => [__('Main background', PPS_LANG_CODE), __('Image 1', PPS_LANG_CODE), __('Top background', PPS_LANG_CODE), __('Button background', PPS_LANG_CODE), __('Email background', PPS_LANG_CODE)],
      46 => [__('Main background', PPS_LANG_CODE), __('Image 1', PPS_LANG_CODE), __('Button background', PPS_LANG_CODE)],
      47 => [__('Main background', PPS_LANG_CODE), __('Top Image', PPS_LANG_CODE), __('Top overlay', PPS_LANG_CODE), __('Bottom background', PPS_LANG_CODE)],
      48 => [__('Main background', PPS_LANG_CODE), __('Image 1', PPS_LANG_CODE), __('Email background', PPS_LANG_CODE), __('Border color', PPS_LANG_CODE)],
      49 => [__('Main background', PPS_LANG_CODE), __('Top background', PPS_LANG_CODE), __('Email background', PPS_LANG_CODE), __('Button background', PPS_LANG_CODE)],
      50 => [__('Additional background', PPS_LANG_CODE), __('Main background', PPS_LANG_CODE), __('Button background', PPS_LANG_CODE)],
      52 => [__('Upper background', PPS_LANG_CODE), __('Main background', PPS_LANG_CODE), __('Yes button background', PPS_LANG_CODE), __('No button background', PPS_LANG_CODE)],
      53 => [__('Main background', PPS_LANG_CODE), __('Image background', PPS_LANG_CODE), __('Button background', PPS_LANG_CODE)],
      54 => [__('Main background', PPS_LANG_CODE), __('Image background', PPS_LANG_CODE), __('Button background', PPS_LANG_CODE)],
      55 => [__('Main background', PPS_LANG_CODE), __('Image background', PPS_LANG_CODE), __('Button background', PPS_LANG_CODE)],
      56 => [__('Main background', PPS_LANG_CODE), __('Frame background', PPS_LANG_CODE)],
      57 => [__('Main background', PPS_LANG_CODE), __('Button background', PPS_LANG_CODE)],
      58 => [__('Main background', PPS_LANG_CODE), __('Field background', PPS_LANG_CODE), __('Button background', PPS_LANG_CODE), __('Left image background', PPS_LANG_CODE)],
      60 => [__('Top background', PPS_LANG_CODE), __('Top overlap background', PPS_LANG_CODE), __('Image 1', PPS_LANG_CODE), __('Image 2', PPS_LANG_CODE), __('Main background', PPS_LANG_CODE), __('Button background', PPS_LANG_CODE), __('Field background', PPS_LANG_CODE)],
      61 => [__('Main background', PPS_LANG_CODE), __('Button background', PPS_LANG_CODE), __('Email background', PPS_LANG_CODE)],
      62 => [__('Main background', PPS_LANG_CODE), __('Image 1', PPS_LANG_CODE), __('Left background', PPS_LANG_CODE), __('E-mail background', PPS_LANG_CODE)],
      63 => [__('Main background', PPS_LANG_CODE), __('Overlay background', PPS_LANG_CODE), __('E-mail background', PPS_LANG_CODE), __('Button background', PPS_LANG_CODE), __('Image', PPS_LANG_CODE)],
      64 => [__('Main background', PPS_LANG_CODE), __('Left background', PPS_LANG_CODE), __('Right background', PPS_LANG_CODE), __('Women button background', PPS_LANG_CODE), __('Men button background', PPS_LANG_CODE)],
      65 => [__('Background 1', PPS_LANG_CODE), __('Background 2', PPS_LANG_CODE), __('Left image', PPS_LANG_CODE), __('Button background', PPS_LANG_CODE)],
      66 => [__('Main background', PPS_LANG_CODE), __('Field background', PPS_LANG_CODE), __('Button background', PPS_LANG_CODE)],
      67 => [
      ...
      68 => [__('Background 1', PPS_LANG_CODE), __('Background 2', PPS_LANG_CODE), __('Field background', PPS_LANG_CODE), __('Button background', PPS_LANG_CODE)],
      69 => [__('Main background', PPS_LANG_CODE), __('Image', PPS_LANG_CODE), __('Image background', PPS_LANG_CODE), __('Field background', PPS_LANG_CODE), __('Submit button background', PPS_LANG_CODE), __('Exit button background', PPS_LANG_CODE)],
      70 => [__('Main background', PPS_LANG_CODE), __('Field background', PPS_LANG_CODE), __('Submit button background', PPS_LANG_CODE), __('Exit button background', PPS_LANG_CODE)],
      71 => [__('Main background', PPS_LANG_CODE), __('Image', PPS_LANG_CODE), __('Field background', PPS_LANG_CODE), __('Submit button background', PPS_LANG_CODE), __('Name icon', PPS_LANG_CODE), __('Email icon', PPS_LANG_CODE), __('Exit button background', PPS_LANG_CODE)],
      72 => [__('Main background', PPS_LANG_CODE), __('Field background', PPS_LANG_CODE), __('Submit button background', PPS_LANG_CODE), __('Name icon', PPS_LANG_CODE), __('Email icon', PPS_LANG_CODE), __('Right Image', PPS_LANG_CODE)],
    ];
  }
