  ...
  jQuery('.popup-list-item').click(function () {
    ...
    if (ppsOriginalPopup.original_id == id) {
      var dialog = jQuery('<div />')
        .html(toeLangPps('This is the same template that was used for Pop-Up before.'))
        .dialog({
          modal: true,
          width: 480,
          height: 180,
...
function _ppsShowPromoPopupForTpl($tplItem) {
  var $proOptWnd = ppsGetMainPromoPopup(),
    selectedTplHref = $tplItem ? jQuery($tplItem).find('a.ppsPromoTplBtn').attr('href') : jQuery('.popup-list-item.active a.ppsPromoTplBtn').attr('href');
