...
function ppsUpdatePopupStatsGraph(chartType, params) {
  if (typeof ppsPopupAllStats != 'undefined') {
    ...
    if (g_ppsCurrentPlot) {
      ((dateFrom = jQuery('#ppsPopupEditForm').find('input[name=stat_from_txt]').val()), (dateTo = jQuery('#ppsPopupEditForm').find('input[name=stat_to_txt]').val()));
    ...
    for (var i = 0; i < ppsPopupAllStats.length; i++) {
      ...
      if (ppsPopupAllStats[i]['points'] && ppsPopupAllStats[i]['points'].length && (dateFrom || dateTo)) {
        ...
        for (var j = 0; j < ppsPopupAllStats[i]['points'].length; j++) {
          ...
          if ((dateFrom && currentDate < dateFrom && !(group == 'week' && currentDate + weekMs > dateFrom) && !(group == 'month' && currentDate + monthMs > dateFrom)) || (dateTo && currentDate > dateTo)) {
            continue;
          }
  ...
  if (g_ppsCurrentStats && g_ppsCurrentStats.length) {
    ...
    if (hasData) {
      ...
      for (var date in g_tblDataToDate) {
        ...
        if (g_tblDataToDate[date]['unique_requests']) g_tblDataToDate[date]['conversion'] = g_tblDataToDate[date]['actions'] / g_tblDataToDate[date]['unique_requests'];
