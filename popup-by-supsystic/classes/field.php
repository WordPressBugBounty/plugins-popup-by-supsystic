...
class fieldPps
{
   ...
   */
  public $adapt = ['htmlPps' => '', 'dbFrom' => '', 'dbTo' => ''];
  /**
    ...
    if (method_exists('htmlPps', $this->html)) {
      ...
      if ($this->name == 'default_value') {
        ...
        if (!empty($optionsFromDb)) {
          $options = [0 => __('Select', PPS_LANG_CODE)];
  ...
  public function viewField($name, $fieldValue = '')
  {
    ...
    $params = ['optionsPps' => $options, 'attrs' => $attrs, 'value' => $fieldValue, 'checked' => $checked];
    $output = '';
    if (method_exists('htmlPps', $method)) {
      $output .= htmlPps::$method($name, $params);
      $output .= htmlPps::hidden('extra_field[' . $this->name . ']', ['value' => $this->id]);
    }
    return $output;
  }
  ...
  public function processParams($tag, $id)
  {
    ...
    if ($this->name == "params") {
      ...
      if ($tag > 0 || $id == 0) {
        ...
        $output .= htmlPps::text('params[height]', ['value' => $height]) . '<br />';
        $output .= '</div>';
      }
