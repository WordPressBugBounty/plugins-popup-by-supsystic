<?php if (in_array($this->popup['type'], [PPS_COMMON, PPS_VIDEO, PPS_AGE_VERIFY, PPS_FULL_SCREEN, PPS_BAR])) { ?>
<div class="ppsPopupOptRow">
  <?php echo viewPps::ksesString(htmlPps::checkbox('params[tpl][enb_label]', ['checked' => htmlPps::checkedOpt($this->popup['params']['tpl'], 'enb_label')])); ?>
  <?php echo viewPps::ksesString(
    htmlPps::text('params[tpl][label]', [
...
<fieldset>
  <legend>
    <label>
      ...
      <?php echo viewPps::ksesString(htmlPps::checkbox('params[tpl][enb_txt_' . $i . ']', ['checked' => htmlPps::checkedOpt($this->popup['params']['tpl'], 'enb_txt_' . $i), 'attrs' => 'data-switch-block="' . $switchBlock . '"'])); ?>
  ...
  <span data-block-to-switch="<?php echo viewPps::ksesString($switchBlock); ?>">
    <?php wp_editor(isset($this->popup['params']['tpl']['txt_' . $i]) ? $this->popup['params']['tpl']['txt_' . $i] : '', 'params_tpl_txt_' . $i, [
...
<fieldset>
  <legend>
    <label>
      ...
      <?php echo viewPps::ksesString(
        htmlPps::checkbox('params[tpl][enb_btn_' . $i . ']', [
  ...
  <span data-block-to-switch="<?php echo viewPps::ksesString($switchBlock); ?>">
    <table>
      <tr>
        ...
        <td><?php echo viewPps::ksesString(htmlPps::text('params[tpl][btn_txt_' . $i . ']', ['value' => isset($this->popup['params']['tpl']['btn_txt_' . $i]) ? $this->popup['params']['tpl']['btn_txt_' . $i] : '', 'attrs' => 'id="' . $btnId . '"'])); ?></td>
      </tr>
      <tr>
        <?php $btnId = 'ppsParamBtnUrlTxt_' . $i; ?>
        <td><label for="<?php echo viewPps::ksesString($btnId); ?>"><?php _e('Button URL', PPS_LANG_CODE); ?>:</label></td>
        <td>
          <?php echo viewPps::ksesString(htmlPps::text('params[tpl][btn_url_' . $i . ']', ['value' => isset($this->popup['params']['tpl']['btn_url_' . $i]) ? $this->popup['params']['tpl']['btn_url_' . $i] : '', 'attrs' => 'id="' . $btnId . '"'])); ?>
          <label>
            <?php _e('or close PopUp on click', PPS_LANG_CODE); ?> -
            <?php echo viewPps::ksesString(
              htmlPps::checkbox('params[tpl][is_close_btn_' . $i . ']', [
      ...
      <tr class="supsystic-tooltip" title="<?php _e('If it is primary - then it will be used when we check "Whom to Show" -> "Until user makes an action" condition.', PPS_LANG_CODE); ?>">
        ...
        <td>
          <?php echo viewPps::ksesString(
            htmlPps::checkbox('params[tpl][is_btn_primary_' . $i . ']', [
...
<?php } ?>
<?php } ?>
<?php if (!in_array($this->popup['type'], [PPS_SIMPLE_HTML, PPS_FULL_SCREEN, PPS_LOGIN_REGISTER])) { ?>
<fieldset>
  <legend>
    <label>
      <?php echo viewPps::ksesString(
        htmlPps::checkbox('params[tpl][enb_foot_note]', [
  ...
  <span data-block-to-switch="txtFooter">
    <?php echo viewPps::ksesString(
      htmlPps::textarea('params[tpl][foot_note]', [
