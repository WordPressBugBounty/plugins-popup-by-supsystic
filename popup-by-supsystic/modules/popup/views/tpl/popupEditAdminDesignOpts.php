<table class="form-table" style="width: auto;">
  <?php if (in_array($this->popup['type'], [PPS_VIDEO])) { ?>
  <tr>
    ...
    <td class="col-w-1perc">
      <?php echo viewPps::ksesString(htmlPps::text('params[tpl][video_url]', ['value' => $this->popup['params']['tpl']['video_url'], 'attrs' => 'style="width: 100%;"'])); ?>
    </td>
  </tr>
  <tr class="ppsVideoVimeoExtraShell">
    ...
    <td class="col-w-1perc">
      <?php echo viewPps::ksesString(
        htmlPps::checkbox('params[tpl][video_extra_full_screen]', [
  ...
  <?php } ?>
  <?php if (in_array($this->popup['type'], [PPS_IFRAME])) { ?>
  <tr>
    <th scope="row" class="col-w-1perc">
      <?php _e('iFrame URL', PPS_LANG_CODE); ?>&nbsp;
      <i class="fa fa-question supsystic-tooltip" title="<?php echo esc_html(
        sprintf(
          __('Copy and paste here URL of site, that you need to display in PopUp as iFrame. Please note that some sites can block such possibility, you can read more about this <a target="_blank" href="%s">for example here</a>', PPS_LANG_CODE),
          'https://developer.mozilla.org/en-US/docs/Web/HTTP/X-Frame-Options',
        ),
      ); ?>"></i>
    </th>
    <td class="col-w-1perc">
      <?php echo viewPps::ksesString(htmlPps::text('params[tpl][iframe_url]', ['value' => $this->popup['params']['tpl']['iframe_url'], 'attrs' => 'style="width: 100%;"'])); ?>
    </td>
  </tr>
  <?php } ?>
  <?php if (in_array($this->popup['type'], [PPS_PDF])) { ?>
  <tr>
    <th scope="row" class="col-w-1perc">
    ...
    <td class="col-w-1perc">
      <?php echo viewPps::ksesString(htmlPps::text('params[tpl][pdf_url]', ['value' => $this->popup['params']['tpl']['pdf_url'], 'attrs' => 'style="width: 100%;"'])); ?>
    </td>
  </tr>
  <?php } ?>
  <tr>
    ...
    <td class="col-w-1perc">
      <?php echo viewPps::ksesString(htmlPps::text('params[tpl][width]', ['value' => $this->popup['params']['tpl']['width']])); ?>
    </td>
    <td class="col-w-1perc" colspan="3">
      <?php if (in_array($this->popup['type'], [PPS_COMMON, PPS_BAR])) { ?>
      <label style="margin-right: 10px;" class="supsystic-tooltip" title="<?php _e('Max width for percentage - is 100', PPS_LANG_CODE); ?>">
        <?php echo viewPps::ksesString(htmlPps::radiobutton('params[tpl][width_measure]', ['value' => '%', 'checked' => htmlPps::checkedOpt($this->popup['params']['tpl'], 'width_measure', '%')])); ?>
        <?php _e('Percents', PPS_LANG_CODE); ?>
      </label>
      <label>
        <?php echo viewPps::ksesString(htmlPps::radiobutton('params[tpl][width_measure]', ['value' => 'px', 'checked' => htmlPps::checkedOpt($this->popup['params']['tpl'], 'width_measure', 'px')])); ?>
        <?php _e('Pixels', PPS_LANG_CODE); ?>
      </label>
      <?php } else {echo viewPps::ksesString(htmlPps::hidden('params[tpl][width_measure]', ['value' => 'px']));} ?>
    </td>
  </tr>
  <?php if (in_array($this->popup['type'], [PPS_FB_LIKE, PPS_VIDEO, PPS_IFRAME, PPS_PDF])) { ?>
  <tr>
    <th scope="row" class="col-w-1perc">
      <?php _e('Height', PPS_LANG_CODE); ?>
    </th>
    <td class="col-w-1perc">
      <?php echo viewPps::ksesString(htmlPps::text('params[tpl][height]', ['value' => $this->popup['params']['tpl']['height']])); ?>
    </td>
    <td class="col-w-1perc" colspan="3">
      <?php echo viewPps::ksesString(htmlPps::hidden('params[tpl][height_measure]', ['value' => 'px'])); ?>
    </td>
  </tr>
  <?php if (in_array($this->popup['type'], [PPS_VIDEO])) { ?>
  <tr>
    ...
    <td class="col-w-1perc">
      <?php echo viewPps::ksesString(
        htmlPps::checkbox('params[tpl][video_autoplay]', [
  ...
  <tr>
    ...
    <td class="col-w-1perc">
      <?php echo viewPps::ksesString(
        htmlPps::checkbox('params[tpl][vide_hide_controls]', [
  ...
  <td class="col-w-1perc">
    ...
    ?>
  <?php } ?>
  <?php if (in_array($this->popup['type'], [PPS_FB_LIKE])) { ?>
  <?php foreach ($this->fbLikeOpts as $fKey => $fData) { ?>
  <?php
  $html = $fData['html'];
  $htmlParams = [];
  if (in_array($html, ['selectbox', 'selectlist'])) {
    $htmlParams['options'] = $fData['options'];
  }
  if ($html == 'checkbox') {
    $htmlParams['checked'] = htmlPps::checkedOpt($this->popup['params']['tpl']['fb_like_opts'], $fKey);
  } else {
    $htmlParams['value'] = isset($this->popup['params']['tpl']['fb_like_opts'][$fKey]) ? $this->popup['params']['tpl']['fb_like_opts'][$fKey] : (isset($fData['def']) ? $fData['def'] : '');
  }
  $htmlParams['attrs'] = '';
  if ($fKey == 'href') {
    $htmlParams['attrs'] = 'style="width: 100%"';
  }
  if (in_array($html, ['selectlist'])) {
  ...
  <tr>
    ...
    <td class="col-w-1perc" colspan="4">
      <?php echo viewPps::ksesString(
        htmlPps::checkbox('params[tpl][fb_enb_label]', [
  ...
  <tr>
    ...
    <td class="col-w-1perc">
      <?php echo viewPps::ksesString(htmlPps::slider('params[tpl][bg_overlay_opacity]', ['value' => $this->popup['params']['tpl']['bg_overlay_opacity'], 'min' => 0, 'max' => 1, 'step' => 0.1])); ?>
    </td>
    <td class="col-w-1perc">
      <?php echo viewPps::ksesString(
        htmlPps::selectbox('params[tpl][bg_overlay_type]', [
          'options' => ['color' => __('Color', PPS_LANG_CODE), 'img' => __('Image', PPS_LANG_CODE), 'snow' => __('Snow', PPS_LANG_CODE)],
    ...
    <td class="col-w-1perc">
      <div class="ppsBgOverlaySets ppsBgOverlaySets_color">
        <?php echo viewPps::ksesString(
          htmlPps::colorpicker('params[tpl][bg_overlay_color]', [
      ...
      <div class="ppsBgOverlaySets ppsBgOverlaySets_img">
        <?php echo viewPps::ksesString(
          htmlPps::imgGalleryBtn('params[tpl][bg_overlay_img]', [
        ...
        <label>
          ...
          <?php echo viewPps::ksesString(
            htmlPps::selectbox('params[tpl][bg_overlay_img_pos]', [
              'options' => ['stretch' => __('Stretch', PPS_LANG_CODE), 'center' => __('Center', PPS_LANG_CODE), 'tile' => __('Tile', PPS_LANG_CODE)],
  ...
  <tr>
    ...
    <td class="col-w-1perc">
      <?php echo viewPps::ksesString(
        htmlPps::checkbox('params[tpl][dsbl_wnd_scroll]', [
  ...
  <tr class="ppsBgRowShell">
    ...
    <td class="col-w-1perc">
      <?php echo viewPps::ksesString(htmlPps::selectbox('params[tpl][bg_type_' . $i . ']', ['options' => $this->bgTypes, 'value' => $this->popup['params']['tpl']['bg_type_' . $i], 'attrs' => 'data-iter="' . $i . '" class="ppsBgTypeSelect"'])); ?>
    </td>
    <td class="col-w-1perc ppsBgTypeShell ppsBgTypeShell_<?php echo esc_html($i); ?> ppsBgTypeImgShell_<?php echo esc_html($i); ?>">
      <?php echo viewPps::ksesString(htmlPps::imgGalleryBtn('params[tpl][bg_img_' . $i . ']', ['onChange' => 'ppsShowImgPrev', 'attrs' => 'data-iter="' . $i . '" class="button button-sup-small"', 'value' => $this->popup['params']['tpl']['bg_img_' . $i]])); ?>
    </td>
    ...
    <td class="col-w-1perc ppsBgTypeShell ppsBgTypeShell_<?php echo esc_html($i); ?> ppsBgTypeColorShell_<?php echo esc_html($i); ?>" style="line-height: 40px;">
      <?php echo viewPps::ksesString(htmlPps::colorpicker('params[tpl][bg_color_' . $i . ']', ['value' => $this->popup['params']['tpl']['bg_color_' . $i]])); ?>
    </td>
  </tr>
  <?php } ?>
  <?php if (!in_array($this->popup['type'], [PPS_FB_LIKE, PPS_IFRAME, PPS_SIMPLE_HTML, PPS_PDF])) { ?>
  <tr>
    <th scope="row" class="col-w-1perc">
    ...
    <td class="col-w-1perc">
      <?php echo viewPps::ksesString(
        htmlPps::fontsList('params[tpl][font_label]', [
          'attrs' => 'class="ppsProOpt"',
    ...
    <td class="col-w-1perc" colspan="3">
      <?php echo viewPps::ksesString(
        htmlPps::colorpicker('params[tpl][label_font_color]', [
          'attrs' => 'class="ppsProOpt"',
  ...
  <tr>
    ...
    <td class="col-w-1perc">
      <?php echo viewPps::ksesString(
        htmlPps::fontsList('params[tpl][font_txt_' . $i . ']', [
          'attrs' => 'class="ppsProOpt"',
    ...
    <td class="col-w-1perc" colspan="3">
      <?php echo viewPps::ksesString(
        htmlPps::colorpicker('params[tpl][text_font_color_' . $i . ']', [
          'attrs' => 'class="ppsProOpt"',
  ...
  <?php } ?>
  <?php } ?>
  <?php if (!in_array($this->popup['type'], [PPS_SIMPLE_HTML, PPS_PDF])) { ?>
  <tr>
    <th scope="row" class="col-w-1perc">
    ...
    <td class="col-w-1perc">
      <?php echo viewPps::ksesString(
        htmlPps::fontsList('params[tpl][font_footer]', [
          'attrs' => 'class="ppsProOpt"',
    ...
    <td class="col-w-1perc" colspan="3">
      <?php echo viewPps::ksesString(
        htmlPps::colorpicker('params[tpl][footer_font_color]', [
          'attrs' => 'class="ppsProOpt"',
  ...
  <tr>
    ...
    <td class="col-w-1perc">
      <label class="supsystic-tooltip" title="<?php _e('PopUp will be fully zoomed for smaller screens', PPS_LANG_CODE); ?>">
        <?php echo viewPps::ksesString(
          htmlPps::radiobutton('params[tpl][responsive_mode]', [
            'value' => 'def',
  ...
  <tr>
    <th scope="row" class="col-w-1perc">
      ...
      <i class="fa fa-question supsystic-tooltip" title="<?php echo esc_html(
        __('If you want - you can redirect user after PopUp will be closed. Just enter required Redirect URL here - and each time after PopUp will be closed - user will be redirected to that URL. Just leave this field empty - if you don\'t need this functionality in your PopUp.', PPS_LANG_CODE),
      ); ?>"></i>
    </th>
    <td colspan="4">
      <?php echo viewPps::ksesString(
        htmlPps::text('params[tpl][reidrect_on_close]', [
      ...
      <label>
        <?php echo viewPps::ksesString(
          htmlPps::checkbox('params[tpl][reidrect_on_close_new_wnd]', [
      ...
      <label class="supsystic-tooltip">
        ...
        <?php echo viewPps::ksesString(
          htmlPps::checkbox('params[tpl][close_redirect_to_btn_url]', [
  ...
  <tr>
    <th scope="row" class="col-w-1perc">
      ...
      <i class="fa fa-question supsystic-tooltip" title="<?php echo esc_html(
        __(
          'You can leave only required part of your iFrame PopUp by entering here CSS selector of this part. Do not use this option if you don\'t understad basics of CSS and HTML. Also please note that you can use this only if your iframe url have same domain that your - due to permissions of html iframe.',
          PPS_LANG_CODE,
        ),
      ); ?>"></i>
      <?php if (!$this->isPro) { ?>
    ...
    <td colspan="4">
      <?php echo viewPps::ksesString(
        htmlPps::text('params[tpl][iframe_display_only]', [
  ...
  <tr>
    ...
    <td colspan="4">
      <ul id="ppsPopupCloseBtnList" class="ppsListItems">
        ...
        <?php if ($this->popup['original_id'] == 52 && $this->popup['type'] == 'age_verify' && $key != 'none') {
          continue;
        } ?>
