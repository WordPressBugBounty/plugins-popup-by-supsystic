...
class htmlPps
{
  ...
  public static function checkboxlist($name, $params = ['options' => [], 'attrs' => '', 'checked' => '', 'delim' => '<br />', 'usetable' => 5], $delim = '<br />')
  {
    $out = '';
    if (!strpos($name, '[]')) {
      $name .= '[]';
  ...
  public static function radiobuttons($name, $params = ['attrs' => '', 'options' => [], 'value' => '', ''])
  {
    ...
    if (isset($params['options']) && is_array($params['options']) && !empty($params['options'])) {
      ...
      foreach ($params['options'] as $key => $val) {
        ...
        $out .= self::input($name, ['attrs' => $params['attrs'] . ' ' . $checked, 'type' => 'radio', 'value' => $key]);
        if ($params['labeled']) {
          $out .= '</label>';
        }
        if (!$params['no_br']) {
  ...
  public static function nonceForFrontend()
  {
    return self::hidden('_wpnonce', ['value' => wp_create_nonce('pps_nonce_frontend')]);
  }
  public static function defaultNonceForAdminPanel()
  {
    return self::hidden('_wpnonce', ['value' => wp_create_nonce('pps_nonce')]);
  }
}
