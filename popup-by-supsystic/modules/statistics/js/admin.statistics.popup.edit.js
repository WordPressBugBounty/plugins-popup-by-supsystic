...
jQuery(document).ready(function(){
	...
	jQuery('#ppsPopupStatClear').click(function(){
		if(confirm(toeLangPps('Are you sure want to clear all PopUp Statistics?'))) {
			jQuery.sendFormPps({
				btn: this
			,	data: {mod: 'statistics', '_wpnonce': PPS_NONCE['pps_nonce'], action: 'clearForPopUp', id: jQuery(this).data('id')}
...
function ppsPopupStatChangeGroup(newGroup, btn) {
	...
	jQuery.sendFormPps({
		btn: btn
	,	data: {mod: 'statistics', '_wpnonce': PPS_NONCE['pps_nonce'], action: 'getUpdatedStats', group: newGroup, id: ppsPopup.id}
	,	onSuccess: function(res) {
