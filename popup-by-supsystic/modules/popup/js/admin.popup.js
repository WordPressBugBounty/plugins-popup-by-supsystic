...
jQuery(document).ready(function () {
  ...
  showForLoggedIn
    ...
    .trigger("change");
  if (typeof ppsOriginalPopup !== "undefined") {
    // Just changing template - for existing popup
    ppsInitChangePopupDialog();
  } else {
    // Creating new popup
    ppsInitCreatePopupDialog();
  }
  if (jQuery(".ppsTplPrevImg").length) {
    // If on creation page
...
function ppsAdjustPreviewSize() {
  ...
  if (shellWidth < startFrom && shellWidth > endFrom) {
    jQuery(".ppsTplPrevImg").css(
      "max-width",
      initialMaxWidth - Math.floor((startFrom - shellWidth) / 2)
    );
...
function ppsInitChangePopupDialog() {
  ...
  jQuery(
    '.popup-list-item[data-id="' + ppsOriginalPopup.original_id + '"]'
  ).addClass("active");
  ...
  jQuery(".popup-list-item").click(function () {
    ...
    if (ppsOriginalPopup.original_id == id) {
      var dialog = jQuery("<div />")
        .html(
          toeLangPps(
            "This is the same template that was used for Pop-Up before."
          )
        )
        .dialog({
          modal: true,
    ...
    jQuery("#ppsChangeTplNewLabel").html(
      jQuery(this).find(".ppsTplLabel").html()
    );
...
function ppsInitCreatePopupDialog() {
  jQuery(".popup-list-item").click(function () {
    ...
    if (id) {
      jQuery("#ppsCreatePopupForm")
        .find("[name=original_id]")
        .val(jQuery(this).data("id"));
    }
    if (id) {
      g_ppsPromoTplSelected = false;
      return false;
    ...
    confirm(
      toeLangPps(
        'Are you sure want to remove "' +
          ppsGetGridColDataById(id, "label", tblId) +
          '" Pop-Up?'
      )
    )
  ) {
