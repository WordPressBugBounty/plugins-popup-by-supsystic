...
class fieldAdapterPps
{
  ...
  public static function taxDataHtml($field)
  {
    ...
    if (!empty($categories)) {
      ...
      foreach ($categories as $c) {
        $cOptions[] = ['id' => $c->term_taxonomy_id,
            'text' => $c->cat_name,
    ...
    if (!empty($brands)) {
      ...
      foreach ($brands as $b) {
        $bOptions[] = ['id' => $b->term_taxonomy_id,
            'text' => $b->cat_name,
    ...
    return '<div>' . __('Apply To', PPS_LANG_CODE) . '
            <div id="tax_address">
                ...
                ' . __('Destination', PPS_LANG_CODE) . ':' . htmlPps::checkboxlist('params[dest]', ['optionsPps' => $listOfDest]) . '<br />
                ' . __('Country', PPS_LANG_CODE) . ':' . htmlPps::countryList('params[country]', ['notSelected' => true, 'value' => $field->value['country']]) . '<br />
            </div>
            <div id="tax_category">
                <b>' . __('Categories', PPS_LANG_CODE) . '</b><br />
                ' . (empty($cOptions) ? __('You have no categories', PPS_LANG_CODE) : htmlPps::checkboxlist('params[categories][]', ['optionsPps' => $cOptions])) . '<br />
                    <b>' . __('Brands', PPS_LANG_CODE) . '</b><br />
                ' . (empty($bOptions) ? __('You have no brands', PPS_LANG_CODE) : htmlPps::checkboxlist('params[brands][]', ['optionsPps' => $bOptions])) . '<br />
            </div>
            <div>' . __('Tax Rate', PPS_LANG_CODE) . ': ' . htmlPps::text('params[rate]', ['value' => $field->value['rate']]) . '</div>
            <div>' . __('Absolute', PPS_LANG_CODE) . ': ' . htmlPps::checkbox('params[absolute]', ['checked' => $field->value['absolute']]) . '</div>
        </div>';
  }
  ...
  {
    return ["Abel", "Abril Fatface", "Aclonica", "Acme", "Actor", "Adamina", "Advent Pro",
