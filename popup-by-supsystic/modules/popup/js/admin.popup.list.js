jQuery(document).ready(function () {
  ...
  jQuery("#" + tblId).jqGrid({
    ...
    onSelectRow: function (rowid, e) {
      var tblId = jQuery(this).attr("id"),
        selectedRowIds = jQuery("#" + tblId).jqGrid(
          "getGridParam",
          "selarrrow"
        ),
  ...
  jQuery("#" + tblId + "EmptyMsg").insertAfter(
    jQuery("#" + tblId + "").parent()
  );
  jQuery("#" + tblId + "").jqGrid("navGrid", "#" + tblId + "Nav", {
    edit: false,
    add: false,
    del: false,
  });
  ...
  jQuery("#ppsPopupRemoveGroupBtn").click(function () {
    var selectedRowIds = jQuery("#ppsPopupTbl").jqGrid(
        "getGridParam",
        "selarrrow"
      ),
      listIds = [];
    for (var i in selectedRowIds) {
      var rowData = jQuery("#ppsPopupTbl").jqGrid(
        "getRowData",
        selectedRowIds[i]
      );
    ...
    if (listIds.length == 1) {
      ...
      var labelCellData = ppsGetGridColDataById(
        listIds[0],
        "label",
        "ppsPopupTbl"
      );
    ...
    var confirmMsg =
      listIds.length > 1
        ? toeLangPps(
            "Are you sur want to remove " + listIds.length + " PopUps?"
          )
