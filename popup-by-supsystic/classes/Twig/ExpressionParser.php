...
class Twig_ExpressionParser
{
    ...
    while (!$stream->test(Twig_Token::PUNCTUATION_TYPE, '}')) {
      ...
      if (($token = $stream->nextIf(Twig_Token::STRING_TYPE)) || ($token = $stream->nextIf(Twig_Token::NAME_TYPE)) || ($token = $stream->nextIf(Twig_Token::NUMBER_TYPE))) {
        ...
        throw new Twig_Error_Syntax(
          sprintf('A hash key must be a quoted string, a number, a name, or an expression enclosed in parentheses (unexpected token "%s" of value "%s"', Twig_Token::typeToEnglish($current->getType()), $current->getValue()),
          $current->getLine(),
  ...
  public function getFunctionNode($name, $line)
  {
    switch ($name) {
      ...
      case 'attribute':
        ...
        return new Twig_Node_Expression_GetAttr($args->getNode(0), $args->getNode(1), count($args) > 2 ? $args->getNode(2) : new Twig_Node_Expression_Array([], $line), Twig_Template::ANY_CALL, $line);
      default:
  ...
  public function parseAssignmentExpression()
  {
    ...
    while (true) {
      ...
      if (in_array($token->getValue(), ['true', 'false', 'none'])) {
