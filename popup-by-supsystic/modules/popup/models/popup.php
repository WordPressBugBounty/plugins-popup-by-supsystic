...
class popupModelPps extends modelPps
{
  ...
  protected function _afterGetFromTbl($row)
  {
    if (isset($row['params'])) {
      ...
      if (!empty($row['params']) && isset($row['params']['tpl']) && !empty($row['params']['tpl'])) {
        $keysCheckEmail = ['sub_txt_subscriber_mail_from', 'sub_new_email', 'sub_txt_confirm_mail_from',
  ...
  public function createFromTpl($d = [])
  {
    ...
    if (!empty($d['label'])) {
      if (!empty($d['original_id'])) {
        ...
        framePps::_()->getModule('supsystic_promo')->getModel()->bigStatAddCheck('Used Template', ['Selected Template' => $original['label']]);
        unset($original['id']);
  ...
  public function remove($id)
  {
    $id = (int) $id;
    if ($id) {
      if (framePps::_()->getTable($this->_tbl)->delete(['id' => $id])) {
        return true;
      } else {
  ...
  {
    if (framePps::_()->getTable($this->_tbl)->delete(['additionalCondition' => 'original_id != 0'])) {
      return true;
    } else {
      $this->pushError(__('Database error detected', PPS_LANG_CODE));
    }
  ...
  private function _bindShowToPages($d)
  {
    ...
    if ($id) {
      ...
      framePps::_()->getTable('popup_show_pages')->delete(['popup_id' => $id]);
      $insertArr = [];
      ...
      framePps::_()->getTable('popup_show_categories')->delete(['popup_id' => $id]);
      $insertArr = [];
  ...
  public function getById($id)
  {
    ...
    if ($data) {
      $data['show_pages_list'] = framePps::_()->getTable('popup_show_pages')->get('*', ['popup_id' => $id]);
      $data['show_categories_list'] = framePps::_()->getTable('popup_show_categories')->get('*', ['popup_id' => $id]);
    }
    return $data;
  }
    ...
    if ($d['id'] && $d['new_tpl_id']) {
      ...
      if (!empty($diffFromOriginal)) {
        if (isset($newTpl['params'])) {
          $keysForMove = ['params.tpl.label', 'params.tpl.anim_key', 'params.tpl.enb_foot_note', 'params.tpl.foot_note',
              'params.tpl.enb_sm',
      ...
      framePps::_()->getModule('supsystic_promo')->getModel()->bigStatAddCheck('Used Template', ['Changed to Template' => $newTpl['label']]);
  ...
  private function _computeDifferences($popup, $original, $key = '', $keysImplode = [])
  {
    $difs = [];
    if (is_array($popup)) {
      $excludeKey = ['id', 'label', 'active', 'original_id', 'img_preview', 'type_id',
      ...
      if (!empty($allPopups)) {
        ...
        foreach ($allPopups as $p) {
          ...
          if (!empty($stats)) {
            ...
            $tbl = $this->getTbl();
            framePps::_()->getTable($tbl)->update([
                'views' => $total['views'],
  ...
  public function switchActive($d = [])
  {
    ...
    if (!empty($d['id'])) {
      $tbl = $this->getTbl();
      return framePps::_()->getTable($tbl)->update([
          'active' => $d['active'],
      ], [
          'id' => $d['id'],
      ]);
