...
class optionsPps extends modulePps
{
  ...
  {
    $tabs['settings'] = [
      'label' => __('Settings', PPS_LANG_CODE),
    ...
    if (empty($this->_options)) {
      ...
      $this->_options = dispatcherPps::applyFilters('optionsDefine', [
        'general' => [
          ...
          'opts' => [
            'disable_subscribe_ip_antispam' => [
              'label' => __('Disable blocking Subscription from same IP', PPS_LANG_CODE),
              'desc' => __('By default our plugin have feature to block subscriptions from same IP more then one time per hour - to avoid spam subscribers. But you can disable this feature here.', PPS_LANG_CODE),
              'def' => '1',
              'html' => 'checkboxHiddenVal',
            ],
            'enable_spam_filter' => [
              'label' => __('Enable additional spam filter', PPS_LANG_CODE),
              'desc' => __('This will enable additional subscribe spam filter. After enable this - just test it from browser, where you are not logged-in - can be conflicts with some cache plugins.', PPS_LANG_CODE),
              'def' => '0',
              'html' => 'checkboxHiddenVal',
            ],
            'disable_autosave' => [
              'label' => __('Disable autosave on PopUp Edit', PPS_LANG_CODE),
            ...
            'access_roles' => [
              ...
              'desc' => __('User with next roles will have access to whole plugin from admin area.', PPS_LANG_CODE),
              'def' => 'administrator',
              'html' => 'selectlist',
            ...
            'foot_assets' => [
              'label' => __('Load Assets in Footer', PPS_LANG_CODE),
              'desc' => __('Force load all plugin CSS and JavaScript files in footer - to increase page load speed. Please make sure that you have correct footer.php file in your WordPress theme with wp_footer() function call in it.', PPS_LANG_CODE),
              'def' => '0',
              'html' => 'checkboxHiddenVal',
            ],
            'disable_email_html_type' => [
              'label' => __('Disable HTML Emails content type', PPS_LANG_CODE),
              'desc' => __('Some servers fail send emails with HTML content type: content-type = "text/html", so if you have problems with sending emails from our plugn - try to disable this feature here.', PPS_LANG_CODE),
              'def' => '0',
              'html' => 'checkboxHiddenVal',
            ],
            'mail_send_engine' => [
              'label' => __('Send With', PPS_LANG_CODE),
              'desc' => __('You can send your emails with different email sendng engines.', PPS_LANG_CODE),
              'def' => 'wp_mail',
              'html' => 'selectbox',
            ...
            'smtp_secure' => [
              'label' => __('SMTP Secure', PPS_LANG_CODE),
              'desc' => __('Use secure SMTP connection. If you enable this option - make sure that your server support such secure connections.', PPS_LANG_CODE),
              'html' => 'selectbox',
              'connect' => 'mail_send_engine:smtp',
              'options' => ['' => __('No', PPS_LANG_CODE), 'ssl' => 'SSL', 'tls' => 'TLS'],
              'def' => '',
            ],
      ...
      foreach ($this->_options as $catKey => $cData) {
        foreach ($cData['opts'] as $optKey => $opt) {
          ...
          if (isset($opt['pro']) && !$isPro) {
            $this->_options[$catKey]['opts'][$optKey]['pro'] = framePps::_()
              ->getModule('supsystic_promo')
              ->generateMainLink('utm_source=plugin&utm_medium=' . $optKey . '&utm_campaign=popup');
          }
        }
      }
