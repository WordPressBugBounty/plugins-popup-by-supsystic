...
jQuery(document).ready(function () {
  ...
  jQuery('#ppsPopupEditForm')
    ...
    .change(function () {
      ...
      if (!PPS_DATA.isPro && typeof g_ppsProSubMethods !== 'undefined' && typeof g_ppsProSubMethods[selectedSubMethod] !== 'undefined') {
  ...
  jQuery('#ppsPopupEditForm [name="params[tpl][sub_dsbl_dbl_opt_id]"]')
    .change(function () {
      jQuery(this).prop('checked') ? jQuery('#ppsSubMcSendWelcome').slideDown(g_ppsAnimationSpeed) : jQuery('#ppsSubMcSendWelcome').slideUp(g_ppsAnimationSpeed);
    })
    .change();
});
      ...
      jQuery.sendFormPps({
        ...
        onSuccess: function (res) {
          if (!res.error) {
            ...
            if (selectedListsIds && selectedListsIds.length) _ppsUpdateMailchimpGroups(selectedListsIds);
          }
        },
      });
    } else {
...
function _ppsSfInitStandardFieldToolbar(cell) {
  if (parseInt(cell.find('input[name*="custom"]').val()) === 0) {
    ...
    toolbarHtml.find('.ppsSfFieldSettingsBtn').click(function () {
      ...
      parseInt(cell.find('input[name*="mandatory"]').val()) ? $wnd.find('[name="mandatory"]').attr('checked', 'checked') : $wnd.find('[name="mandatory"]').removeAttr('checked');

      ppsCheckUpdateArea($wnd);
      g_ppsSfCurrEditStandardCell = cell;
      name == 'email' ? $wnd.find('.ppsSfMandatoryStandardRow').hide() : $wnd.find('.ppsSfMandatoryStandardRow').show();
      $wnd.dialog('open');
      return false;
    });
  }
