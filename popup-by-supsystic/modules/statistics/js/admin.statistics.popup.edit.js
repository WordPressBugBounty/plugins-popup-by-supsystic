...
jQuery(document).ready(function () {
  ...
  jQuery("#ppsPopupStatClear").click(function () {
    if (
      confirm(toeLangPps("Are you sure want to clear all PopUp Statistics?"))
    ) {
      jQuery.sendFormPps({
        btn: this,
        data: {
...
function ppsPopupStatSetChartType(type) {
  ...
  jQuery('.ppsPopupStatChartTypeBtn[data-type="' + type + '"]').addClass(
    "focus"
  );
    ...
    if (typeof ppsPopupAllStats != "undefined") {
      ...
      for (var i = 0; i < ppsPopupAllStats.length; i++) {
        if (
          ...
          plotDataByCode[labelCode] = {
            label: ppsPopupAllStats[i].label,
            total: 0,
          };
          for (var j = 0; j < ppsPopupAllStats[i]["points"].length; j++) {
            plotDataByCode[labelCode].total += parseInt(
              ppsPopupAllStats[i]["points"][j]["total_requests"]
            );
          }
          haveData = true;
        }
      ...
      if (haveData) {
        ...
        for (var code in plotDataByCode) {
          plotData.push([
            plotDataByCode[code].label,
            plotDataByCode[code].total,
          ]);
        }
...
function ppsUpdateAllShareChart() {
  if (!g_ppsPieAllShareDone) {
    ...
    } else jQuery("#ppsPopupStatAllShareNoData").show();
...
function ppsUpdatePopupStatsGraph(chartType, params) {
  if (typeof ppsPopupAllStats != "undefined") {
    ...
    if (g_ppsCurrentPlot) {
      ((dateFrom = jQuery("#ppsPopupEditForm")
        .find("input[name=stat_from_txt]")
        .val()),
        (dateTo = jQuery("#ppsPopupEditForm")
          .find("input[name=stat_to_txt]")
          .val()));
    ...
    for (var i = 0; i < ppsPopupAllStats.length; i++) {
      ...
      if (
        ...
        currentData["points"] = [];
        for (var j = 0; j < ppsPopupAllStats[i]["points"].length; j++) {
          var currentDate = ppsStrToMs(
            ppsPopupAllStats[i]["points"][j]["date"]
          );
          if (
            (dateFrom &&
      ...
      case "bar":
        var ticksKeys = {},
          ...
          sortByDateClb = function (a, b) {
            var aTime = new Date(
                str_replace(typeof a === "string" ? a : a.date, "-", "/")
              ).getTime(), // should be no "-" as ff make it Date.parse() in incorrect way
              bTime = new Date(
                str_replace(typeof b === "string" ? b : b.date, "-", "/")
              ).getTime();
        ...
        for (var i = 0; i < g_ppsCurrentStats.length; i++) {
          if (
            g_ppsCurrentStats[i]["points"] &&
            g_ppsCurrentStats[i]["points"].length
          ) {
            plotDataToDate.push({});
            for (
              var j = g_ppsCurrentStats[i]["points"].length - 1;
              j >= 0;
              j--
            ) {
              ticksKeys[g_ppsCurrentStats[i]["points"][j]["date"]] = 1;
              plotDataToDate[tickId][
                g_ppsCurrentStats[i]["points"][j]["date"]
              ] = parseInt(g_ppsCurrentStats[i]["points"][j]["total_requests"]);
            }
        ...
        for (var i = 0; i < plotDataToDate.length; i++) {
          ...
          for (var j in ticks) {
            var dateStr = ticks[j];
            plotData[tickId].push(
              typeof plotDataToDate[i][dateStr] === "undefined"
                ? 0
                : plotDataToDate[i][dateStr]
            );
          }
          tickId++;
        }
        ...
        g_ppsCurrentPlot = jQuery.jqplot(
          "ppsPopupStatGraph",
          plotData,
          plotParams
        );
    ...
    if (firstInit) {
      g_ppsCurrentPlot.target.bind(
        "jqplotZoom",
        function (ev, gridpos, datapos, plot, cursor) {
...
function ppsPopupStatRebuildTbl() {
  ...
  jQuery("#" + tblId).jqGrid({
    ...
    colNames: [
      toeLangPps("Date"),
      toeLangPps("Views"),
      toeLangPps("Unique Views"),
      toeLangPps("Actions"),
      toeLangPps("Conversion"),
    ],
    colModel: [
      {
        name: "date",
      ...
      if (
        ...
        for (var j = 0; j < g_ppsCurrentStats[i]["points"].length; j++) {
          ...
          if (toeInArrayPps(g_ppsCurrentStats[i]["code"], ["show"])) {
            currentData["views"] = parseInt(
              g_ppsCurrentStats[i]["points"][j]["total_requests"]
            );
          } else if (
            !toeInArrayPps(g_ppsCurrentStats[i]["code"], [
              "close",
              "subscribe_error",
            ])
          ) {
            currentData["actions"] = parseInt(
              g_ppsCurrentStats[i]["points"][j]["total_requests"]
            );
          }
          var uniqueRequests = parseInt(
            g_ppsCurrentStats[i]["points"][j]["unique_requests"]
          );
          if (uniqueRequests) {
          ...
          if (g_tblDataToDate[date]) {
            ...
            currentData["unique_requests"] +=
              g_tblDataToDate[date]["unique_requests"];
          }
    ...
    if (hasData) {
      ...
      for (var date in g_tblDataToDate) {
        ...
        if (g_tblDataToDate[date]["unique_requests"])
          g_tblDataToDate[date]["conversion"] =
            g_tblDataToDate[date]["actions"] /
            g_tblDataToDate[date]["unique_requests"];
        g_tblDataToDate[date]["conversion"] =
          g_tblDataToDate[date]["conversion"].toFixed(3);
      }
      for (var date in g_tblDataToDate) {
