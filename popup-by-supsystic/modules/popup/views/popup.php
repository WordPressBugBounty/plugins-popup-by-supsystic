...
class popupViewPps extends viewPps
{
  ...
  public function getTabContent()
  {
    ...
    framePps::_()->addJSVar('admin.popup.list', 'ppsTblDataUrl', uriPps::mod('popup', 'getListForTbl', ['reqType' => 'ajax', '_wpnonce' => wp_create_nonce('pps_nonce')]));
  ...
  public function getAddNewTabContent()
  {
    ...
    if ($changeFor) {
      ...
      dispatcherPps::addFilter('mainBreadcrumbs', [$this, 'modifyBreadcrumbsForChangeTpl']);
    }
    $this->assign('types', $this->_getTypesForList());
    $this->assign(
      'list',
      dispatcherPps::applyFilters(
        'showTplsList',
        $this->getModel()
          ->setOrderBy('sort_order')
          ->setSortOrder('ASC')
          ->getSimpleList(['active' => 1, 'original_id' => 0]),
      ),
    );
  ...
  private function _getTypesForList()
  {
    ...
    $addTypes = [
      101 => ['code' => 'gmaps', 'label' => __('Google Maps', PPS_LANG_CODE), 'fective' => [29, 31], 'replace' => 1],
    ];
  ...
  public function getEditTabContent($id)
  {
    ...
    dispatcherPps::addAction('afterAdminBreadcrumbs', [$this, 'showEditPopupFormControls']);
    dispatcherPps::addAction('adminBreadcrumbsClassAdd', [$this, 'adminBreadcrumbsClassAdd']);

    $useCommonTabs = in_array($popup['type'], [PPS_COMMON, PPS_VIDEO, PPS_BAR]);
    // !remove this!!!!
    ...
    $this->assign(
      ...
      framePps::_()->getModule('add_options')
        ? framePps::_()
          ->getModule('add_options')
          ->showPopupShortcode(['id' => $id])
    ...
    $postTypesForPostsList = ['page', 'post', 'product', 'blog', 'grp_pages', 'documentation'];
    ...
    $selectedShowPages = $selectedHidePages = [];
    ...
    $allCategories = dbPps::get("SELECT t.term_id, t.name FROM $wpdb->terms t " . "INNER JOIN $wpdb->term_taxonomy tt ON t.term_id = tt.term_id " . "WHERE tt.taxonomy IN ('" . implode("','", $postTypesForCategoriesList) . "') ORDER BY t.name");
    ...
    $selectedShowCategories = $selectedHideCategories = [];
    ...
    $popupGroups = !empty($popupGroups['general']['opts']['groups']['value']) ? $popupGroups['general']['opts']['groups']['value'] : [];
    ...
    if (in_array($popup['type'], [PPS_FB_LIKE])) {
      $this->assign('fbLikeOpts', $this->getFbLikeOpts());
    }
    $this->assign('ppsAddNewUrl', $ppsAddNewUrl);
    $this->assign('bgTypes', $bgType);
    $this->assign('previewUrl', uriPps::mod('popup', 'getPreviewHtml', ['id' => $id, 'baseUrl' => admin_url(), '_wpnonce' => wp_create_nonce('pps_nonce')]));
    ...
    $designTabs = [
      // Used in $this->getMainPopupTplTab()
      'ppsPopupDesign' => [
    ...
    if ($useCommonTabs || in_array($popup['type'], [PPS_SIMPLE_HTML, PPS_AGE_VERIFY, PPS_FULL_SCREEN, PPS_LOGIN_REGISTER])) {
    ...
    uasort($designTabs, [$this, 'sortEditPopupTabsClb']);
  ...
  public function getFbLikeOpts()
  {
    return [
      ...
      'tabs' => [
        ...
        'options' => ['timeline' => __('Timeline', PPS_LANG_CODE), 'events' => __('Events', PPS_LANG_CODE), 'messages' => __('Messages', PPS_LANG_CODE)],
  ...
  public function getMainPopupContactFormTab()
  {
    ...
    if ($contactFormSupported) {
      $contactFormsListForSelect = [];
      global $wpdb;
      $contactForms = $wpdb->get_results("SELECT id, label FROM {$wpdb->prefix}cfs_forms WHERE original_id != 0 AND ab_id = 0", ARRAY_A);
  ...
  public function getMainPopupSubTab()
  {
    ...
    $this->assign('wpCsvExportUrl', uriPps::mod('subscribe', 'getWpCsvList', ['id' => $this->popup['id'], '_wpnonce' => wp_create_nonce('pps_nonce_export')]));
  ...
  public function getMainPopupSmTab()
  {
    ...
    if ($sssPlugAvailable && isset($supsysticSocialSharing) && method_exists($supsysticSocialSharing, 'getEnvironment')) {
      ...
      if (empty($sssProjects)) {
        ...
        $sssProjectsForSelect = [0 => __('None - use Standard PopUp Social Buttons')];
        $popupIdFound = false;
        foreach ($sssProjects as $p) {
          $sssProjectsForSelect[$p->id] = $p->title;
          if (isset($p->settings) && isset($p->settings['popup_id']) && $p->settings['popup_id'] == $this->popup['id']) {
        ...
        if (!$popupIdFound && isset($this->popup['params']['tpl']['use_sss_prj_id']) && !empty($this->popup['params']['tpl']['use_sss_prj_id'])) {
          $this->popup['params']['tpl']['use_sss_prj_id'] = 0;
        }
      }
      $sssProjectsForSelect = !empty($sssProjectsForSelect) ? $sssProjectsForSelect : [];
  ...
  public function getAnimationList()
  {
    if (empty($this->_animationList)) {
      $this->_animationList = [
        'none' => ['label' => __('None', PPS_LANG_CODE)],
        'puff' => ['label' => __('Puff', PPS_LANG_CODE), 'show_class' => 'puffIn', 'hide_class' => 'puffOut'],
        'vanish' => ['label' => __('Vanish', PPS_LANG_CODE), 'show_class' => 'vanishIn', 'hide_class' => 'vanishOut'],

        'open_down_left' => ['label' => __('Open down left', PPS_LANG_CODE), 'show_class' => 'openDownLeftRetourn', 'hide_class' => 'openDownLeft'],
        'open_down_right' => ['label' => __('Open down right', PPS_LANG_CODE), 'show_class' => 'openDownRightRetourn', 'hide_class' => 'openDownRight'],

        'perspective_down' => ['label' => __('Perspective down', PPS_LANG_CODE), 'show_class' => 'perspectiveDownRetourn', 'hide_class' => 'perspectiveDown'],
        'perspective_up' => ['label' => __('Perspective up', PPS_LANG_CODE), 'show_class' => 'perspectiveUpRetourn', 'hide_class' => 'perspectiveUp'],

        'slide_down' => ['label' => __('Slide down', PPS_LANG_CODE), 'show_class' => 'slideDownRetourn', 'hide_class' => 'slideDown'],
        'slide_up' => ['label' => __('Slide up', PPS_LANG_CODE), 'show_class' => 'slideUpRetourn', 'hide_class' => 'slideUp'],

        'swash' => ['label' => __('Swash', PPS_LANG_CODE), 'show_class' => 'swashIn', 'hide_class' => 'swashOut'],
        'foolis' => ['label' => __('Foolis', PPS_LANG_CODE), 'show_class' => 'foolishIn', 'hide_class' => 'foolishOut'],

        'tin_right' => ['label' => __('Tin right', PPS_LANG_CODE), 'show_class' => 'tinRightIn', 'hide_class' => 'tinRightOut'],
        'tin_left' => ['label' => __('Tin left', PPS_LANG_CODE), 'show_class' => 'tinLeftIn', 'hide_class' => 'tinLeftOut'],
        'tin_up' => ['label' => __('Tin up', PPS_LANG_CODE), 'show_class' => 'tinUpIn', 'hide_class' => 'tinUpOut'],
        'tin_down' => ['label' => __('Tin down', PPS_LANG_CODE), 'show_class' => 'tinDownIn', 'hide_class' => 'tinDownOut'],

        'boing' => ['label' => __('Boing', PPS_LANG_CODE), 'show_class' => 'boingInUp', 'hide_class' => 'boingOutDown'],

        'space_right' => ['label' => __('Space right', PPS_LANG_CODE), 'show_class' => 'spaceInRight', 'hide_class' => 'spaceOutRight'],
        'space_left' => ['label' => __('Space left', PPS_LANG_CODE), 'show_class' => 'spaceInLeft', 'hide_class' => 'spaceOutLeft'],
        'space_up' => ['label' => __('Space up', PPS_LANG_CODE), 'show_class' => 'spaceInUp', 'hide_class' => 'spaceOutUp'],
        'space_down' => ['label' => __('Space down', PPS_LANG_CODE), 'show_class' => 'spaceInDown', 'hide_class' => 'spaceOutDown'],
      ];
      ...
      $this->_animationList = array_merge($this->_animationList, [
        'bounce' => ['label' => __('Bounce', PPS_LANG_CODE), 'show_class' => 'bounceIn', 'hide_class' => 'bounceOut'],
        'bounce_up' => ['label' => __('Bounce Up', PPS_LANG_CODE), 'show_class' => 'bounceInUp', 'hide_class' => 'bounceOutUp'],
        'bounce_down' => ['label' => __('Bounce Down', PPS_LANG_CODE), 'show_class' => 'bounceInDown', 'hide_class' => 'bounceOutDown'],
        'bounce_left' => ['label' => __('Bounce Left', PPS_LANG_CODE), 'show_class' => 'bounceInLeft', 'hide_class' => 'bounceOutLeft'],
        'bounce_right' => ['label' => __('Bounce Right', PPS_LANG_CODE), 'show_class' => 'bounceInRight', 'hide_class' => 'bounceOutRight'],

        'fade' => ['label' => __('Fade', PPS_LANG_CODE), 'show_class' => 'fadeIn', 'hide_class' => 'fadeOut'],
        'fade_up' => ['label' => __('Fade Up', PPS_LANG_CODE), 'show_class' => 'fadeInUp', 'hide_class' => 'fadeOutUp'],
        'fade_down' => ['label' => __('Fade Down', PPS_LANG_CODE), 'show_class' => 'fadeInDown', 'hide_class' => 'fadeOutDown'],
        'fade_left' => ['label' => __('Fade Left', PPS_LANG_CODE), 'show_class' => 'fadeInLeft', 'hide_class' => 'fadeOutLeft'],
        'fade_right' => ['label' => __('Fade Right', PPS_LANG_CODE), 'show_class' => 'fadeInRight', 'hide_class' => 'fadeOutRight'],

        'flip_x' => ['label' => __('Flip X', PPS_LANG_CODE), 'show_class' => 'flipInX', 'hide_class' => 'flipOutX'],
        'flip_y' => ['label' => __('Flip Y', PPS_LANG_CODE), 'show_class' => 'flipInY', 'hide_class' => 'flipOutY'],

        'rotate' => ['label' => __('Rotate', PPS_LANG_CODE), 'show_class' => 'rotateIn', 'hide_class' => 'rotateOut'],
        'rotate_up_left' => ['label' => __('Rotate Up Left', PPS_LANG_CODE), 'show_class' => 'rotateInUpLeft', 'hide_class' => 'rotateOutUpLeft'],
        'rotate_up_right' => ['label' => __('Rotate Up Right', PPS_LANG_CODE), 'show_class' => 'rotateInUpRight', 'hide_class' => 'rotateOutUpRight'],
        'rotate_down_left' => ['label' => __('Rotate Down Left', PPS_LANG_CODE), 'show_class' => 'rotateInDownLeft', 'hide_class' => 'rotateOutDownLeft'],
        'rotate_down_right' => ['label' => __('Rotate Down Right', PPS_LANG_CODE), 'show_class' => 'rotateInDownRight', 'hide_class' => 'rotateOutDownRight'],

        'slide_up' => ['label' => __('Slide Up', PPS_LANG_CODE), 'show_class' => 'slideInUp', 'hide_class' => 'slideOutUp'],
        'slide_down' => ['label' => __('Slide Down', PPS_LANG_CODE), 'show_class' => 'slideInDown', 'hide_class' => 'slideOutDown'],
        'slide_left' => ['label' => __('Slide Left', PPS_LANG_CODE), 'show_class' => 'slideInLeft', 'hide_class' => 'slideOutLeft'],
        'slide_right' => ['label' => __('Slide Right', PPS_LANG_CODE), 'show_class' => 'slideInRight', 'hide_class' => 'slideOutRight'],

        'zoom' => ['label' => __('Zoom', PPS_LANG_CODE), 'show_class' => 'zoomIn', 'hide_class' => 'zoomOut'],
        'zoom_up' => ['label' => __('Zoom Up', PPS_LANG_CODE), 'show_class' => 'zoomInUp', 'hide_class' => 'zoomOutUp'],
        'zoom_down' => ['label' => __('Zoom Down', PPS_LANG_CODE), 'show_class' => 'zoomInDown', 'hide_class' => 'zoomOutDown'],
        'zoom_left' => ['label' => __('Zoom Left', PPS_LANG_CODE), 'show_class' => 'zoomInLeft', 'hide_class' => 'zoomOutLeft'],
        'zoom_right' => ['label' => __('Zoom Right', PPS_LANG_CODE), 'show_class' => 'zoomInRight', 'hide_class' => 'zoomOutRight'],

        'light_speed' => ['label' => __('Light Speed', PPS_LANG_CODE), 'show_class' => 'lightSpeedIn', 'hide_class' => 'lightSpeedOut'],
        'roll' => ['label' => __('Rolling!', PPS_LANG_CODE), 'show_class' => 'rollIn', 'hide_class' => 'rollOut'],
      ]);
    }
  ...
  {
    if (isset($popup['params']['tpl']['close_btn']) && !empty($popup['params']['tpl']['close_btn']) && $popup['params']['tpl']['close_btn'] !== 'none') {
      $this->getCloseBtns();
  ...
  {
    if (isset($popup['params']['tpl']['bullets']) && !empty($popup['params']['tpl']['bullets']) && $popup['params']['tpl']['bullets'] !== 'none') {
      $this->getBullets();
  ...
  private function _generateVideoHtml($popup)
  {
    ...
    if (isset($popup['params']['tpl']['video_url']) && !empty($popup['params']['tpl']['video_url'])) {
      ...
      if (
        isset($popup['params']['tpl']['video_autoplay']) &&
        $popup['params']['tpl']['video_autoplay'] &&
        // For youtube we will autoplay it from js
        (!isset($popup['params']['tpl']['video_type']) || !in_array($popup['params']['tpl']['video_type'], [PPS_VIDEO_YOUTUBE]))
      ) {
        $attrs['autoplay'] = 1;
      }
      ...
      add_filter('oembed_result', [$this, 'modifyEmbRes'], 10, 3);
  ...
  private function _generateSelfHostedVideo($url, $attrs)
  {
    ...
    if (isset($attrs['width'])) {
      $attrsArr['width'] = $attrs['width'];
    }
    if (isset($attrs['height'])) {
      $attrsArr['height'] = $attrs['height'];
    }
  ...
  public function generateHtml($popup, $params = [])
  {
    ...
    if (isset($popup['params']['tpl']['enb_contact_form']) && !empty($popup['params']['tpl']['enb_contact_form'])) {
      ...
      if ($contactFormsSupported) {
        $popup['params']['tpl']['sub_form_start'] = frameCfs::_()
          ->getModule('forms')
          ->showForm(['id' => $popup['params']['tpl']['contact_form']]);
      } else {
      ...
      (isset($popup['params']['tpl']['enb_subscribe']) && !empty($popup['params']['tpl']['enb_subscribe'])) ||
    ...
    if (
      (!isset($popup['params']['tpl']['enb_subscribe']) || empty($popup['params']['tpl']['enb_subscribe'])) &&
      ((isset($popup['params']['tpl']['enb_login']) && !empty($popup['params']['tpl']['enb_login'])) || (isset($popup['params']['tpl']['enb_reg']) && !empty($popup['params']['tpl']['enb_reg'])))
    ) {
    ...
    if (in_array($popup['type'], [PPS_VIDEO])) {
    ...
    return $this->_twig->render(($replaceStyleTag ? '<span style="display: none;" id="ppsPopupStylesHidden_' . $popup['view_id'] . '">' : '<style type="text/css">') . $popup['css'] . ($replaceStyleTag ? '</span>' : '</style>') . $popup['html'], ['popup' => $popup]);
  }
  ...
  private function _generateFbLikeWidget($popup)
  {
    ...
    $res .=
      ...
      utilsPps::getLangCode() .
      '/sdk.js#xfbml=1&version=v2.5&appId=1612081092370131";
    ...
    foreach ($fbLikeOpts as $fKey => $fData) {
      ...
      if ($fData['html'] == 'checkbox') {
        $value = isset($popup['params']['tpl']['fb_like_opts'][$fKey]) && $popup['params']['tpl']['fb_like_opts'][$fKey] ? 'true' : 'false';
      } else {
        $value = isset($popup['params']['tpl']['fb_like_opts'][$fKey]) ? $popup['params']['tpl']['fb_like_opts'][$fKey] : (isset($fData['def']) ? $fData['def'] : '');
      }
      if (is_array($value)) {
        $value = implode(',', $value);
      }
  ...
  private function _replaceTagsWithTwig($string, &$popup)
  {
    ...
    $replaceFrom = ['ID', 'endif', 'else', 'wp_logout_url'];
    $replaceTo = [$popup['view_id'], '{% endif %}', '{% else %}', wp_logout_url()];
    ...
    add_shortcode('embed', [$this, 'processEmbedCode']);
    ...
    remove_shortcode('embed', [$this, 'processEmbedCode']);
  ...
  public function getCloseBtns()
  {
    if (empty($this->_closeBtns)) {
      $this->_closeBtns = [
        'none' => ['label' => __('None', PPS_LANG_CODE)],
        'classy_grey' => ['img' => 'classy_grey.png', 'add_style' => ['top' => '-16px', 'right' => '-16px', 'width' => '42px', 'height' => '42px']],
        'close-orange' => ['img' => 'close-orange.png', 'add_style' => ['top' => '-16px', 'right' => '-16px', 'width' => '42px', 'height' => '42px']],
        'close-red-in-circle' => ['img' => 'close-red-in-circle.png', 'add_style' => ['top' => '-16px', 'right' => '-16px', 'width' => '42px', 'height' => '42px']],
        'exclusive_close' => ['img' => 'exclusive_close.png', 'add_style' => ['top' => '-10px', 'right' => '-35px', 'width' => '31px', 'height' => '31px']],
        'lists_black' => ['img' => 'lists_black.png', 'add_style' => ['top' => '-10px', 'right' => '-10px', 'width' => '25px', 'height' => '25px']],
        'while_close' => ['img' => 'while_close.png', 'add_style' => ['top' => '15px', 'right' => '15px', 'width' => '20px', 'height' => '19px']],
        'red_close' => ['img' => 'close-red.png', 'add_style' => ['top' => '15px', 'right' => '20px', 'width' => '25px', 'height' => '25px']],
        'yellow_close' => ['img' => 'close-yellow.png', 'add_style' => ['top' => '-16px', 'right' => '-16px', 'width' => '42px', 'height' => '42px']],
        'sqr_close' => ['img' => 'sqr-close.png', 'add_style' => ['top' => '25px', 'right' => '20px', 'width' => '25px', 'height' => '25px']],
        'close-black-in-white-circle' => ['img' => 'close-black-in-white-circle.png', 'add_style' => ['top' => '16px', 'right' => '16px', 'width' => '32px', 'height' => '32px']],
        'circle_big_new_close' => ['img' => 'circle_big_new_close.png', 'add_style' => ['top' => '0', 'right' => '0', 'width' => '31px', 'height' => '31px']],
      ];
  ...
  public function getBullets()
  {
    if (empty($this->_bullets)) {
      $this->_bullets = [
        'none' => ['label' => __('None (standard)', PPS_LANG_CODE)],
        'classy_blue' => ['img' => 'classy_blue.png', 'add_style' => ['list-style' => 'none', 'background-repeat' => 'no-repeat', 'padding-left' => '30px', 'line-height' => '100%', 'margin-bottom' => '10px', 'min-height' => '18px']],
        'circle_green' => ['img' => 'circle_green.png', 'add_style' => ['list-style' => 'none', 'background-repeat' => 'no-repeat', 'padding-left' => '30px', 'min-height' => '18px']],
        'lists_green' => ['img' => 'lists_green.png', 'add_style' => ['list-style' => 'none', 'background-repeat' => 'no-repeat', 'padding-left' => '30px', 'margin-bottom' => '10px', 'min-height' => '25px']],
        'tick' => ['img' => 'tick.png', 'add_style' => ['list-style' => 'none', 'background-repeat' => 'no-repeat', 'padding-left' => '30px', 'line-height' => '100%', 'margin-bottom' => '10px', 'min-height' => '18px']],
        'tick_blue' => ['img' => 'tick_blue.png', 'add_style' => ['list-style' => 'none', 'background-repeat' => 'no-repeat', 'padding-left' => '30px', 'line-height' => '100%', 'margin-bottom' => '10px', 'min-height' => '18px']],
        'ticks' => ['img' => 'ticks.png', 'add_style' => ['list-style' => 'none', 'background-repeat' => 'no-repeat', 'padding-left' => '30px', 'line-height' => '100%', 'margin-bottom' => '10px', 'min-height' => '18px']],
        'pop_icon' => ['img' => 'pop_icon.jpg', 'add_style' => ['list-style' => 'none', 'background-repeat' => 'no-repeat', 'padding-left' => '30px', 'line-height' => '100%', 'background-position' => 'left center', 'margin-bottom' => '15px']],
        'circle_big_new' => ['img' => 'circle_big_new.png', 'add_style' => ['list-style' => 'none', 'background-repeat' => 'no-repeat', 'padding-left' => '40px', 'line-height' => '100%', 'background-position' => 'left center', 'margin-bottom' => '5px', 'line-height' => '30px']],
      ];
      foreach ($this->_bullets as $key => $data) {
        ...
        $this->_bullets[$key]['rtl_style'] = ['padding-left' => '0', 'padding-right' => '30px', 'background-position' => 'top right'];
      }
    }
  ...
  protected function _initTwig()
  {
    if (!$this->_twig) {
      ...
      $this->_twig->addFunction(new Twig_SimpleFunction('hex_to_rgba_str', 'utilsPps::hexToRgbaStr'));
    }
  }
}
