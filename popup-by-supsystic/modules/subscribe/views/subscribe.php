...
class subscribeViewPps extends viewPps
{
  ...
  {
    $redirectUrl = isset($popup['params']['tpl']['sub_redirect_url']) && !empty($popup['params']['tpl']['sub_redirect_url']) ? $popup['params']['tpl']['sub_redirect_url'] : false;
    ...
    $res = '';
    $res .= htmlPps::hidden('listname', ['value' => $popup['params']['tpl']['sub_aweber_listname']]);
    $res .= htmlPps::hidden('meta_message', ['value' => '1']);
    $res .= htmlPps::hidden('meta_required', ['value' => 'email']);
    $res .= htmlPps::hidden('redirect', ['value' => $redirectUrl]);
    if (isset($popup['params']['tpl']['sub_aweber_adtracking']) && !empty($popup['params']['tpl']['sub_aweber_adtracking'])) {
      $res .= htmlPps::hidden('meta_adtracking', ['value' => $popup['params']['tpl']['sub_aweber_adtracking']]);
    }
    $res .= '</form>';
    return $res;
  }
  ...
  private function _generateFormStartCommon($popup, $key = '')
  {
    ...
    if (in_array($popup['original_id'], [31])) {
      // For those templates - put message up to the form
  ...
  {
    $res = '';
    $res .= htmlPps::hidden('mod', ['value' => 'subscribe']);
    $res .= htmlPps::hidden('action', ['value' => 'subscribe']);
    $res .= htmlPps::hidden('id', ['value' => $popup['id']]);
    //$res .= htmlPps::hidden('from_url', array('value' => uriPps::getFullUrl()));
    $res .= htmlPps::hidden('_wpnonce', ['value' => wp_create_nonce('subscribe-' . $popup['id'])]);
    if (!in_array($popup['original_id'], [31])) {
      // For those templates - put message up to the form
      $res .= '<div class="ppsSubMsg"></div>';
    }
