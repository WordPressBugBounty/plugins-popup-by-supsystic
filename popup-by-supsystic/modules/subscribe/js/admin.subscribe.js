...
jQuery(document).ready(function () {
  ...
  jQuery("#ppsPopupEditForm")
    ...
    .change(function () {
      ...
      if (toeInArray(selectedSubMethod, ["aweber"]) === -1) {
        // For aweber we use simple post send to aweber side method - so all messages is generated by aweber
      ...
      if (toeInArray(selectedSubMethod, ["wordpress"]) !== -1) {
        // Only for wordpress - we will handle emails about subscribe confirm
        jQuery(".ppsPopupSubEmailTxt").slideDown(g_ppsAnimationSpeed);
      } else {
        jQuery(".ppsPopupSubEmailTxt").slideUp(g_ppsAnimationSpeed);
      }
      if (toeInArray(selectedSubMethod, ["wordpress", "aweber"]) !== -1) {
        // Create WP subscriber with usual subscription methods
      ...
      if (
        ...
        var promoLink = jQuery(".ppsPopupSubDestOpts_" + selectedSubMethod)
          .find(".ppsProOptMiniLabel a")
          .attr("href");
  ...
  jQuery("#ppsPopupEditForm")
    .find('[name="params[tpl][sub_mailchimp_api_key]"]')
    .change(function () {
      _ppsUpdateMailchimpLists();
    });
  jQuery("#ppsPopupEditForm")
    .find('[name="params[tpl][sub_mailchimp_lists][]"]')
    .change(function () {
  ...
  jQuery("#ppsMailchimpGroups").change(function () {
    ...
    if (groups && groups != "" && groups.length) {
      var $groupsFull = jQuery("#ppsPopupEditForm").find(
          '[name="params[tpl][sub_mailchimp_groups_full]"]'
        ),
        fullArr = [];
  ...
  jQuery(".ppsTestEmailFuncBtn").click(function () {
    jQuery.sendFormPps({
      ...
      data: {
        ...
        action: "testEmail",
        test_email: jQuery("input[name=test_email]").val(),
      },
  ...
  jQuery('#ppsPopupEditForm [name="params[tpl][sub_dsbl_dbl_opt_id]"]')
    .change(function () {
...
function _ppsUpdateMailchimpLists() {
  if (
    jQuery("#ppsPopupEditForm").find('[name="params[tpl][sub_dest]"]').val() ==
    "mailchimp"
  ) {
        ...
        onSuccess: function (res) {
          if (!res.error) {
            ...
            var selectedListsIds =
              ppsPopup &&
              ppsPopup.params.tpl &&
              ppsPopup.params.tpl.sub_mailchimp_lists
                ? ppsPopup.params.tpl.sub_mailchimp_lists
                : [];
            //var allListsForSave = [];
            for (var listId in res.data.lists) {
              var selected = toeInArrayPps(listId, selectedListsIds)
                ? 'selected="selected"'
                : "";
              jQuery("#ppsMailchimpLists").append(
                "<option " +
                  selected +
                  ' value="' +
                  listId +
                  '">' +
                  res.data.lists[listId] +
                  "</option>"
              );
            }
        ...
        onSuccess: function (res) {
          if (!res.error) {
            ...
            var selectedGroupsIds =
              ppsPopup &&
              ppsPopup.params.tpl &&
              ppsPopup.params.tpl.sub_mailchimp_groups
                ? ppsPopup.params.tpl.sub_mailchimp_groups
                : [];
            if (res.data.groups && res.data.groups != {}) {
              for (var groupId in res.data.groups) {
                var selected = toeInArrayPps(groupId, selectedGroupsIds)
                  ? 'selected="selected"'
                  : "";
                jQuery("#ppsMailchimpGroups").append(
                  "<option " +
                    selected +
                    ' value="' +
                    groupId +
                    '">' +
                    res.data.groups[groupId] +
                    "</option>"
                );
              }
            }
            jQuery("#ppsMailchimpGroupsShell").show();
            jQuery("#ppsMailchimpGroups")
              .chosen()
...
function ppsInitStandardSubFieldsPopup() {
  var $wnd = jQuery("#ppsSfEditFieldsStandardWnd").dialog({
    ...
    buttons: {
      Ok: function () {
        if (
          _ppsSfSaveStandrdSubField(
            $wnd.serializeAnythingPps(false, true),
            $wnd
          )
        ) {
          $wnd.dialog("close");
...
function _ppsSfSaveStandrdSubField(data, $wnd) {
  ...
  if (data.label) {
    ...
    ppsSavePopupChanges();
    return true;
  } else errors["label"] = toeLangPps("Please enter Label");

  if ($wnd && errors) {
    toeProcessAjaxResponsePps(
      { error: true, errors: errors },
      false,
      $wnd,
      true,
      { btn: $wnd.find(".ui-button:first") }
    );
  }
  return false;
}
function _ppsSfFillInSubFieldStandardCell(cell, data) {
  cell
    .find('[name="params[tpl][sub_fields][' + data.name + '][label]"]')
    .val(data.label);
  if (data.name != "email") {
    cell
      .find('[name="params[tpl][sub_fields][' + data.name + '][mandatory]"]')
      .val(data.mandatory ? 1 : 0);
  }
  cell.find(".ppsSubFieldLabel").html(data.label);
}
function _ppsSfClearStandardEditForm() {
  jQuery("#ppsSfEditFieldsStandardWnd")
  ...
  jQuery("#ppsSfEditFieldsStandardWnd")
    .find(".ppsSfFieldSelectOptShell:not(#ppsSfFieldSelectOptShellExl)")
    .remove();
}
function _ppsSfInitStandardFieldToolbar(cell) {
  if (parseInt(cell.find('input[name*="custom"]').val()) === 0) {
    var toolbarHtml = jQuery("#ppsSfFieldToolbarStandardExl")
      .clone()
      .removeAttr("id");
