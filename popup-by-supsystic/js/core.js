if (typeof PPS_DATA == "undefined") var PPS_DATA = {};
...
jQuery.fn.sendFormPps = function (params) {
  ...
  if (msgEl && !params.btn) {
    jQuery(msgEl).removeClass("ppsSuccessMsg").removeClass("ppsErrorMsg");
    if (!params.btn) {
  ...
  if (params.btn) {
    ...
    // Font awesome usage
    params.btnIconElement = jQuery(params.btn).find(".fa").length
      ? jQuery(params.btn).find(".fa")
      : jQuery(params.btn);
...
jQuery.fn.animateRemovePps = function (animationSpeed, onSuccess) {
  animationSpeed =
    animationSpeed == undefined ? PPS_DATA.animationSpeed : animationSpeed;
...
function toeProcessAjaxResponsePps(res, msgEl, form, sentFromForm, params) {
  ...
  if (params.btn) {
    ...
    if (params.btnIconElement) {
      params.btnIconElement.attr(
        "class",
        params.btnIconElement.data("prev-class")
      );
    }
  }
  /*if(sentFromForm) {
  ...
  if (typeof res == "object") {
    if (res.error) {
      if (msgEl) {
        jQuery(msgEl).removeClass("ppsSuccessMsg").addClass("ppsErrorMsg");
      }
      var errorsArr = [];
      for (var name in res.errors) {
        ...
        if (
          jQuery(".ppsErrorForField.toe_" + nameToClassId(name) + "").exists()
        )
          jQuery(".ppsErrorForField.toe_" + nameToClassId(name) + "")
            .show()
      ...
      if (errorsArr.length && params.btn && jQuery.fn.dialog && !msgEl) {
        jQuery('<div title="' + toeLangPps("Really small warning :)") + '" />')
      ...
      if (msgEl) {
        jQuery(msgEl).removeClass("ppsErrorMsg").addClass("ppsSuccessMsg");
...
function toeOptionPps(key) {
  if (PPS_DATA.options && PPS_DATA.options[key]) return PPS_DATA.options[key];
  return false;
}
...
function toePagesPps(key) {
  if (typeof PPS_DATA != "undefined" && PPS_DATA[key]) return PPS_DATA[key];
  return false;
}
/**
...
function toeOptShowDescriptionPps(description, x, y, moveToLeft) {
  if (typeof description != "undefined" && description != "") {
    ...
    if (moveToLeft)
      jQuery("#ppsOptDescription").css(
        "right",
        jQuery(window).width() - (x - 10)
      ); // Show it on left side of target
    else jQuery("#ppsOptDescription").css("left", x + 10);
  ...
  options = jQuery.extend(
    {
      ...
      open: function (event, ui) {
        ...
        jQuery(".ui-dialog-titlebar-close")
          .css({
            background:
              'url("' +
              PPS_DATA.cssPath +
              'img/tb-close.png") no-repeat scroll 0 0 transparent',
            border: "0",
            width: "15px",
            height: "15px",
            padding: "0",
        ...
        jQuery(".ui-widget-overlay").css({
          "z-index":
            jQuery(event.target).parents(".ui-dialog:first").css("z-index") - 1,
          "background-image": "none",
        });
...
function ppsReloadCoreJs(clb, params) {
  ...
  for (var i = 0; i < coreScripts.length; i++) {
    scriptsHtml +=
      '<script type="text/javascript" class="ppsReloadedScript" src="' +
      PPS_DATA.jsPath +
      coreScripts[i] +
      '"></script>';
  }
