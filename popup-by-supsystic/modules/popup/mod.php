...
{
  private $_renderedIds = [];
  private $_addToFooterIds = [];
  ...
  public function init()
  {
    ...
    add_action($mainCheckActionName, [$this, 'checkPopupShow']);
    add_shortcode(PPS_SHORTCODE_CLICK, [$this, 'showPopupOnClick']);
    add_action('wp_footer', [$this, 'collectFooterRender'], 0);
  ...
  {
    if ($popup['original_id'] == 67) {
      // Xmas Discount template
  ...
  public function addAdminTab($tabs)
  {
    $tabs[$this->getCode() . '_add_new'] = [
      'label' => __('Add New PopUp', PPS_LANG_CODE),
    ...
    $tabs[$this->getCode()] = [
      'label' => __('Show All PopUps', PPS_LANG_CODE),
  ...
  {
    if (is_admin()) {
      $condition = ['show_in_admin_area' => 1];
  ...
  public function checkPopupShow()
  {
    ...
    if ($currentPageId && $havePostsListing && !$isOnlyOnePage && is_home()) {
    ...
    if ($havePostsListing) {
      ...
      foreach ($wp_query->posts as $post) {
        if (is_object($post) && isset($post->post_content)) {
          if (
            /*preg_match_all('/\[\s*'. PPS_SHORTCODE_CLICK. '.+id\s*\=.*(?P<POPUP_ID>\d+)\]/iUs', $post->post_content, $matches)
            ...
            if (!is_array($matches['POPUP_ID'])) {
              $matches['POPUP_ID'] = [$matches['POPUP_ID']];
            }
  ...
  private function _beforeRender($popups)
  {
    ...
    if (!empty($popups)) {
      ...
      foreach ($popups as $i => $p) {
        if (isset($p['params']['main']['hide_for_devices']) && !empty($p['params']['main']['hide_for_devices'])) {
          ...
          if ((!$hideShowRevert && in_array('mobile', $p['params']['main']['hide_for_devices']) && $isMobile) || ($hideShowRevert && !in_array('mobile', $p['params']['main']['hide_for_devices']) && $isMobile)) {
            unset($popups[$i]);
            $dataRemoved = true;
          } elseif ((!$hideShowRevert && in_array('tablet', $p['params']['main']['hide_for_devices']) && $isTablet) || ($hideShowRevert && !in_array('tablet', $p['params']['main']['hide_for_devices']) && $isTablet)) {
            unset($popups[$i]);
            $dataRemoved = true;
          } elseif ((!$hideShowRevert && in_array('desktop', $p['params']['main']['hide_for_devices']) && $isDesktop) || ($hideShowRevert && !in_array('desktop', $p['params']['main']['hide_for_devices']) && $isDesktop)) {
            unset($popups[$i]);
            $dataRemoved = true;
          }
        }
        if (isset($p['params']['main']['hide_for_post_types']) && !empty($p['params']['main']['hide_for_post_types'])) {
          ...
          if (
            ...
            ((!$hideShowRevert && count($wp_query->posts) === 1 && in_array($postType, $p['params']['main']['hide_for_post_types'])) || ($hideShowRevert && (!in_array($postType, $p['params']['main']['hide_for_post_types']) || count($wp_query->posts) !== 1)))
          ) {
            unset($popups[$i]);
            $dataRemoved = true;
          }
        }
        if (isset($p['params']['main']['hide_for_logged_in']) && !empty($p['params']['main']['hide_for_logged_in']) && $isUserLoggedIn) {
          // Check if we need to hide it from logged-in users
          unset($popups[$i]);
          $dataRemoved = true;
        }
        if (isset($p['params']['main']['show_for_logged_in']) && !empty($p['params']['main']['show_for_logged_in']) && !$isUserLoggedIn) {
          // Check if we need to hide it from logged-in users
          unset($popups[$i]);
          $dataRemoved = true;
        }
        if (isset($p['params']['main']['hide_for_ips']) && !empty($p['params']['main']['hide_for_ips'])) {
          ...
          if (!empty($hideForIpsArr)) {
            ...
            if ((!$hideShowRevert && in_array($userIp, $hideForIpsArr)) || ($hideShowRevert && !in_array($userIp, $hideForIpsArr))) {
              unset($popups[$i]);
              $dataRemoved = true;
            }
          }
        }
        if (isset($p['params']['main']['hide_for_countries']) && !empty($p['params']['main']['hide_for_countries'])) {
          ...
          if ((!$hideShowRevert && in_array($countryCode, $p['params']['main']['hide_for_countries'])) || ($hideShowRevert && !in_array($countryCode, $p['params']['main']['hide_for_countries']))) {
            unset($popups[$i]);
            $dataRemoved = true;
          }
        }
        if (isset($p['params']['main']['hide_for_languages']) && !empty($p['params']['main']['hide_for_languages'])) {
          ...
          if ((!$hideShowRevert && in_array($langCode, $p['params']['main']['hide_for_languages'])) || ($hideShowRevert && !in_array($langCode, $p['params']['main']['hide_for_languages']))) {
            unset($popups[$i]);
            $dataRemoved = true;
          }
        }
        if (isset($p['params']['main']['enb_show_date']) && !empty($p['params']['main']['enb_show_date'])) {
          // Check if we need to show it in Date range
          $tsFrom = isset($p['params']['main']['show_date_from']) && !empty($p['params']['main']['show_date_from']) ? strtotime($p['params']['main']['show_date_from']) : false;
          $tsTo = isset($p['params']['main']['show_date_to']) && !empty($p['params']['main']['show_date_to']) ? strtotime($p['params']['main']['show_date_to']) : false;
          if (!$ts) {
            $ts = time();
          }
          if (($tsFrom && $ts < $tsFrom) || ($tsTo && $ts > $tsTo)) {
            unset($popups[$i]);
            $dataRemoved = true;
          }
        }
        if (isset($p['params']['main']['enb_show_days']) && !empty($p['params']['main']['enb_show_days'])) {
          // Check if we need to show it in selected Days
          $showDays = isset($p['params']['main']['show_days']) ? $p['params']['main']['show_days'] : [];
  ...
  public function renderList($popups, $jsListVarName = 'ppsPopups', $inFooter = false)
  {
    ...
    foreach ($popups as $i => $p) {
      ...
      $popups[$i]['rendered_html'] = $this->getView()->generateHtml($p, ['replace_style_tag' => true]);
    ...
    if (!$renderedBefore) {
      ...
      framePps::_()->addScript('frontend.popup', $this->getModPath() . 'js/frontend.popup' . $assetSuf . '.js', [], '', $inFooter);
      if ($popupGroups) {
        ...
        framePps::_()->addScript('frontend.owl.carousel', PPS_JS_PATH . '/owl.carousel.min.js', [], '', $inFooter);
      }
      ...
      framePps::_()->addScript('frontend.dummy.popup', $this->getModPath() . 'js/frontend.dummy.popup.js', [], '', $inFooter);
  ...
  public function collectFooterRender()
  {
    if (!empty($this->_addToFooterIds)) {
      ...
      foreach ($this->_addToFooterIds as $id) {
        if ((!empty($this->_renderedIds) && in_array($id, $this->_renderedIds)) || in_array($id, $idsToRender)) {
          continue;
        }
        $idsToRender[] = $id;
      }
      if (!empty($idsToRender)) {
        $popups = $this->_beforeRender(
          $this->getModel()
            ->addWhere('id IN (' . implode(',', $idsToRender) . ')')
            ->getFromTbl(),
        );
        if (!empty($popups)) {
          $popups = dispatcherPps::applyFilters('popupListBeforeRender', $popups);
  ...
  public function showPopupOnClick($params)
  {
    ...
    if (!empty($popup)) {
      ...
      $this->renderList([$popup], 'ppsPopupsFromFooter', true);
    }
  ...
  public function checkMenuItemsForPopUps($menuItems)
  {
    if (!empty($menuItems)) {
      foreach ($menuItems as $item) {
        ...
        } elseif (isset($item->url) && !empty($item->url) && strpos($item->url, '#ppsShowPopUp_') !== false) {
          // And in Menu Link
          $checkItem = $item->url;
        }
        if ($checkItem) {
